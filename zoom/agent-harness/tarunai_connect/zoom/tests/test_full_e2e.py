...
2. Valid tokens saved via 'tarunai-connect-zoom auth login'

To run: python3 -m pytest tarunai_connect/zoom/tests/test_full_e2e.py -v

Set TARUNAI_CONNECT_ZOOM_E2E=1 to enable these tests.
"""
...
pytestmark = pytest.mark.skipif(
    not os.environ.get("TARUNAI_CONNECT_ZOOM_E2E"),
    reason="Set TARUNAI_CONNECT_ZOOM_E2E=1 to run E2E tests (requires Zoom credentials)",
)
...
class TestMeetingLifecycleE2E:
    ...
    def test_meeting_crud(self, runner):
        ...
        result = runner.invoke(cli, [
            "--json", "meeting", "create",
            "--topic", "tarunAI Connect E2E Test Meeting",
            "--duration", "15",
            "--timezone", "UTC",
        ])
        assert result.exit_code == 0
        ...
        assert meeting["topic"] == "tarunAI Connect E2E Test Meeting"
        ...
        try:
            ...
            assert info["topic"] == "tarunAI Connect E2E Test Meeting"

            # Update
            result = runner.invoke(cli, [
                "--json", "meeting", "update", str(meeting_id),
                "--topic", "tarunAI Connect Updated Meeting",
                "--duration", "30",
            ])
            assert result.exit_code == 0
            ...
            assert updated["topic"] == "tarunAI Connect Updated Meeting"
