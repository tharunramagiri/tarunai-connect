...
This document describes the testing strategy for the `tarunai-connect-wiremock` package. Tests live in this directory and cover both unit-level logic and end-to-end CLI invocation.
...
pytest tarunai_connect/wiremock/tests/test_core.py -v
```
...
```bash
pytest tarunai_connect/wiremock/tests/test_full_e2e.py -v
```
...
```bash
pytest --cov=tarunai_connect.wiremock --cov-report=term-missing tarunai_connect/wiremock/tests/
```
