"""tarunai-connect-godot — Agent-native CLI for the Godot game engine.
...
from tarunai_connect.godot.utils.godot_backend import (
    get_version,
    is_available,
    find_godot_binary,
)
...
def project_create(ctx, path, name):
    ...
    from tarunai_connect.godot.core.project import create_project
...
def project_info(ctx):
    ...
    from tarunai_connect.godot.core.project import get_project_info
...
def project_scenes(ctx):
    ...
    from tarunai_connect.godot.core.project import list_scenes
...
def project_scripts(ctx):
    ...
    from tarunai_connect.godot.core.project import list_scripts
...
def project_resources(ctx):
    ...
    from tarunai_connect.godot.core.project import list_resources
...
def project_reimport(ctx):
    ...
    from tarunai_connect.godot.core.project import reimport_project
...
def scene_create(ctx, scene_path, root_type, root_name):
    ...
    from tarunai_connect.godot.core.scene import create_scene
...
def scene_read(ctx, scene_path):
    ...
    from tarunai_connect.godot.core.scene import read_scene
...
def scene_add_node(ctx, scene_path, node_name, node_type, parent):
    ...
    from tarunai_connect.godot.core.scene import add_node
...
def export_build(ctx, preset, output, debug):
    ...
    from tarunai_connect.godot.core.export import export_project
...
def export_presets(ctx):
    ...
    from tarunai_connect.godot.core.export import list_export_presets
...
def script_run(ctx, script_path, timeout):
    ...
    from tarunai_connect.godot.core.script import run_script
...
def script_inline(ctx, code, timeout):
    ...
    """
    from tarunai_connect.godot.core.script import run_inline
...
def script_validate(ctx, script_path):
    ...
    from tarunai_connect.godot.core.script import validate_script
...
def session(ctx):
    ...
    try:
        from tarunai_connect.godot.utils.repl_skin import ReplSkin
        skin = ReplSkin("godot", version="1.0.0")
        skin.print_banner()
    except ImportError:
        skin = None
        click.secho("tarunai-connect-godot REPL", fg="green", bold=True)
