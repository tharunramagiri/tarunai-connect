"""Unit tests for tarunai-connect-godot — no Godot binary required.
...
from tarunai_connect.godot.godot_cli import cli
...
class TestEngineStatus:
    def test_engine_status_no_godot(self, runner):
        with mock.patch(
            "tarunai_connect.godot.utils.godot_backend.find_godot_binary",
            return_value=None,
        ):
    ...
    def test_engine_status_found(self, runner):
        with mock.patch(
            "tarunai_connect.godot.godot_cli.find_godot_binary",
            return_value="/usr/bin/godot",
        ):
            with mock.patch(
                "tarunai_connect.godot.godot_cli.is_available",
                return_value=True,
            ):
                result = runner.invoke(cli, ["--json", "engine", "status"])
                data = json.loads(result.output)
...
class TestBackend:
    def test_validate_project(self, tmp_project):
        from tarunai_connect.godot.utils.godot_backend import validate_project
    ...
    def test_find_godot_binary_env(self):
        from tarunai_connect.godot.utils.godot_backend import find_godot_binary
...
class TestCLIRoot:
    def test_help(self, runner):
        ...
        assert result.exit_code == 0
        assert "tarunai-connect-godot" in result.output
