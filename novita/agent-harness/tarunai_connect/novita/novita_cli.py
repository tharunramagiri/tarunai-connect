...
Usage:
    # One-shot commands
    tarunai-connect-novita chat --prompt "Hello" --model deepseek/deepseek-v3.2
...
from tarunai_connect.novita.core.session import ChatSession
from tarunai_connect.novita.utils.novita_backend import (
    get_api_key,
    load_config,
    save_config,
    chat_completion,
...
def get_session():
    global _session
    if _session is None:
        sf = str(Path.home() / ".tarunai-connect-novita" / "session.json")
...
def config_path():
    ...
    from tarunai_connect.novita.utils.novita_backend import CONFIG_FILE
...
def repl():
    ...
    from tarunai_connect.novita.utils.repl_skin import ReplSkin
