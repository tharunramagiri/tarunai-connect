...
Verified on 2026-04-07 in the local tarunAI Connect worktree.

### Commands Run

```bash
uv run --with pytest --with click --with prompt-toolkit --with-editable "c:\Users\lishun\py_ws\提交pr\dify-workflow-cli" python -m pytest tarunai_connect/dify_workflow/tests -vv -s
```
