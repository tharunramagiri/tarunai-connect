...
from tarunai_connect.dify_workflow.dify_workflow_cli import cli
from tarunai_connect.dify_workflow.utils.dify_workflow_backend import (
    build_command,
    has_upstream_cli,
...
class TestBackendDiscovery:
    def test_require_command_returns_binary_path(self):
        with patch("tarunai_connect.dify_workflow.utils.dify_workflow_backend.shutil.which", return_value="/usr/bin/dify-workflow"):
            assert require_dify_workflow_command() == ["/usr/bin/dify-workflow"]

    def test_require_command_falls_back_to_python_module(self):
        with patch("tarunai_connect.dify_workflow.utils.dify_workflow_backend.shutil.which", return_value=None), patch(
            "tarunai_connect.dify_workflow.utils.dify_workflow_backend.importlib.util.find_spec",
            return_value=object(),
        ), patch("tarunai_connect.dify_workflow.utils.dify_workflow_backend.sys.executable", "python"):
    ...
    def test_build_command_appends_args(self):
        with patch("tarunai_connect.dify_workflow.utils.dify_workflow_backend.require_dify_workflow_command", return_value=["dify-workflow"]):
            assert build_command(["guide", "-j"]) == ["dify-workflow", "guide", "-j"]

    def test_has_upstream_cli_false_when_resolution_fails(self):
        with patch("tarunai_connect.dify_workflow.utils.dify_workflow_backend.require_dify_workflow_command", side_effect=RuntimeError("missing")):
...
class TestWrapperCLI:
    def test_help_renders(self):
        ...
        assert "tarunAI Connect wrapper for the Dify workflow DSL editor" in result.output
...
class TestPackagingFixtures:
    ...
    def test_skill_file_documents_wrapper_behavior(self):
        ...
        assert "tarunai-connect-dify-workflow" in skill
