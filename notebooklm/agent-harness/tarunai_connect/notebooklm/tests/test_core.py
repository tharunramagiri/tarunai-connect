...
from tarunai_connect.notebooklm.core.session import Session
from tarunai_connect.notebooklm.utils.notebooklm_backend import (
    build_command,
...
class TestBackendDiscovery:
    def test_require_notebooklm_returns_path(self):
        with patch("tarunai_connect.notebooklm.utils.notebooklm_backend.shutil.which", return_value="/usr/local/bin/notebooklm"):
            assert require_notebooklm() == "/usr/local/bin/notebooklm"

    def test_require_notebooklm_raises_with_install_guidance(self):
        with patch("tarunai_connect.notebooklm.utils.notebooklm_backend.shutil.which", return_value=None):
...
class TestRunNotebooklm:
    def test_run_notebooklm_rejects_json_for_unsupported_command(self):
        with patch("tarunai_connect.notebooklm.utils.notebooklm_backend.subprocess.run") as run_mock:
...
class TestPackagingFixtures:
    def test_acknowledgements_reference_external_projects(self):
        readme = Path("tarunai_connect/notebooklm/README.md").read_text(encoding="utf-8")
        assert "tarunAI Connect" in readme
    ...
    def test_skill_file_contains_usage_and_boundary_guidance(self):
        skill = Path("tarunai_connect/notebooklm/skills/SKILL.md").read_text(encoding="utf-8")
    ...
    def test_skill_file_exists(self):
        assert Path("tarunai_connect/notebooklm/skills/SKILL.md").is_file()
