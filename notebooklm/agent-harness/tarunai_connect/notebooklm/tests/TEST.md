...
```bash
python3 -m pytest tarunai_connect/notebooklm/tests/test_core.py -q
python3 -m pytest tarunai_connect/notebooklm/tests/test_cli_smoke.py -q
python3 -m tarunai_connect.notebooklm.notebooklm_cli --help
```
...
- `python3 -m tarunai_connect.notebooklm.notebooklm_cli --help`: exit code 0, help text rendered correctly

### Notes

- Added a regression test for module execution so `python -m tarunai_connect.notebooklm.notebooklm_cli --help` is covered, not just Click's in-process `CliRunner`.
...
```bash
python3 -m pytest tarunai_connect/notebooklm/tests -q
python3 -m tarunai_connect.notebooklm.notebooklm_cli --help
```
...
python3 -m pytest tarunai_connect/notebooklm/tests/test_cli_smoke.py -q
python3 -m pytest tarunai_connect/notebooklm/tests -q
python3 -m tarunai_connect.notebooklm.notebooklm_cli --json auth login
```
