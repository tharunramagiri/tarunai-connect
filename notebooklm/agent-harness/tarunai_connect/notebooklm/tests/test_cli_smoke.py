...
from tarunai_connect.notebooklm.notebooklm_cli import cli
...
def _resolve_cli(name):
    ...
    module = "tarunai_connect.notebooklm.notebooklm_cli"
...
class TestCommandRouting:
    def test_auth_status_routes_to_auth_check(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "tarunai_connect.notebooklm.notebooklm_cli.run_notebooklm",
            fake_run,
        )
...
class TestModuleExecution:
    def test_python_m_module_help_emits_output(self):
        result = subprocess.run(
            [
                sys.executable,
                "-m",
                "tarunai_connect.notebooklm.notebooklm_cli",
                "--help",
            ],
            capture_output=True,
            text=True,
    ...
    def test_resolved_cli_help_emits_output(self):
        command, env = _resolve_cli("tarunai-connect-notebooklm")
        result = subprocess.run(
            command + ["--help"],
            capture_output=True,
            text=True,
