...
from tarunai_connect.blender.core import scene as scene_mod
from tarunai_connect.blender.core import objects as obj_mod
from tarunai_connect.blender.core import materials as mat_mod
from tarunai_connect.blender.core import modifiers as mod_mod
from tarunai_connect.blender.core import lighting as light_mod
from tarunai_connect.blender.core import animation as anim_mod
from tarunai_connect.blender.core import render as render_mod
...
def _spawn_live_poller(session_dir: str) -> dict:
    ...
    cli_path = shutil.which("tarunai-connect-blender")
    if cli_path:
        ...
        command = [
            sys.executable,
            "-m",
            "tarunai_connect.blender.blender_cli",
            "preview",
            "live",
            "monitor",
            "--session-dir",
...
def preview_capture(recipe, force, root_dir):
    ...
    result = preview_mod.capture(
        ...
        force=force,
        root_dir=root_dir,
        command=f"tarunai-connect-blender --project {sess.project_path or ''} preview capture --recipe {recipe}".strip(),
    )
...
def preview_live_start(recipe, force, root_dir, poll_ms, mode, source_poll_ms, open_window):
    ...
    result = preview_mod.live_start(
        ...
        command=(
            f"tarunai-connect-blender --project {sess.project_path or ''} "
...
def repl(project_path):
    ...
    from tarunai_connect.blender.utils.repl_skin import ReplSkin
