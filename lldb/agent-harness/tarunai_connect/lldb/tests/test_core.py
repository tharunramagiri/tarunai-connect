...
def _resolve_cli(name: str):
    ...
    return [sys.executable, "-m", "tarunai_connect.lldb.lldb_cli"]


class TestOutputUtils:
    def test_output_json(self):
        from tarunai_connect.lldb.utils.output import output_json
        import io
    ...
    def test_output_table(self):
        from tarunai_connect.lldb.utils.output import output_table
        import io
...
class TestErrorUtils:
    def test_handle_error(self):
        from tarunai_connect.lldb.utils.errors import handle_error
...
class TestDAPProtocol:
    def test_encode_and_read_message(self):
        from tarunai_connect.lldb.dap import encode_message, read_message
    ...
    def test_read_message_rejects_missing_content_length(self):
        from tarunai_connect.lldb.dap import DAPProtocolError, read_message
    ...
    def test_initialize_capabilities_and_event(self):
        from tarunai_connect.lldb.dap import LLDBDebugAdapter, encode_message, read_message
    ...
    def test_variable_references_reset_on_resume(self):
        from tarunai_connect.lldb.dap import LLDBDebugAdapter
    ...
    def test_running_state_emits_continued_not_stopped(self):
        from tarunai_connect.lldb.dap import LLDBDebugAdapter, read_message
    ...
    def test_pause_request_interrupts_process_and_reports_stop(self):
        ...
        adapter.run(
            io.BytesIO(
                b"".join(
                    [
                        __import__("tarunai_connect.lldb.dap", fromlist=["encode_message"]).encode_message(
...
class TestCoreHelpers:
    def test_breakpoints_wrapper(self):
        from tarunai_connect.lldb.core.breakpoints import set_breakpoint
    ...
    def test_inspect_wrapper(self):
        from tarunai_connect.lldb.core.inspect import evaluate_expression
    ...
    def test_threads_wrapper(self):
        from tarunai_connect.lldb.core.threads import list_threads
...
class TestCLIHelp:
    def test_main_help(self):
        from tarunai_connect.lldb.lldb_cli import cli
...
class TestCLIJsonErrors:
    @patch("tarunai_connect.lldb.lldb_cli._get_session")
...
class TestBackend:
    @patch("tarunai_connect.lldb.utils.lldb_backend.subprocess.run")
    @patch("tarunai_connect.lldb.utils.lldb_backend.os.path.isdir", return_value=False)
    def test_backend_probe_failure(self, _mock_isdir, mock_run):
        from tarunai_connect.lldb.utils import lldb_backend
    ...
    @patch("tarunai_connect.lldb.utils.lldb_backend.subprocess.run", side_effect=FileNotFoundError())
...
class TestSessionLifecycle:
    def _make_session(self):
        from tarunai_connect.lldb.core.session import LLDBSession
    ...
    def test_find_memory_rejects_oversized_scan(self):
        from tarunai_connect.lldb.core.session import LLDBSession, MEMORY_FIND_MAX_SCAN_SIZE
...
class TestSessionDaemonSecurity:
    def test_state_file_is_written_with_restrictive_mode(self, tmp_path):
        from tarunai_connect.lldb.utils.session_server import _write_state_file
    ...
    def test_session_server_rejects_unknown_methods(self):
        from tarunai_connect.lldb.utils.session_server import SessionServer
...
class TestCLISubprocess:
    CLI_BASE = _resolve_cli("tarunai-connect-lldb")
