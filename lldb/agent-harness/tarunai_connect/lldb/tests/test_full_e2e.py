...
class DAPClient:
    def __init__(self):
        from tarunai_connect.lldb.dap import read_message

        self._read_message = read_message
        self.proc = subprocess.Popen(
            [sys.executable, "-m", "tarunai_connect.lldb.dap"],
    ...
    def request(self, command: str, arguments: dict | None = None, timeout: int = 30):
        from tarunai_connect.lldb.dap import encode_message
...
class TestLLDBDAPE2E:
    def test_dap_source_line_breakpoint(self, lldb_test_exe: str):
        ...
        with DAPClient() as client:
            client.request("initialize", {"adapterID": "tarunai-connect-lldb"})
    ...
    def test_dap_breakpoint_variables_source_disassemble_and_continue(self, lldb_test_exe: str):
        with DAPClient() as client:
            initialize, _ = client.request("initialize", {"adapterID": "tarunai-connect-lldb"})
