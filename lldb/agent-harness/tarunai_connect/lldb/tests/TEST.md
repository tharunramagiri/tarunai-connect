...
```bash
python -m pytest tarunai_connect/lldb/tests/test_core.py -v
python -m pytest tarunai_connect/lldb/tests/test_full_e2e.py -v -s
python -m pytest tarunai_connect/lldb/tests -q
```
...
- Verified the installed `tarunai-connect-lldb` entrypoint on Windows after editable install
