...
def default_session_root() -> Path:
    env_override = os.environ.get("TARUNAI_CONNECT_LLDB_SESSION_DIR")
    if env_override:
    ...
    if os.name == "nt":
        ...
        return (root / "tarunai-connect-lldb" / "sessions").resolve()

    runtime_dir = os.environ.get("XDG_RUNTIME_DIR")
    if runtime_dir:
        return (Path(runtime_dir).expanduser() / "tarunai-connect-lldb").resolve()
    return (Path.home() / ".cache" / "tarunai-connect-lldb" / "sessions").resolve()
...
def resolve_session_file(explicit: str | None = None) -> Path:
    ...
    env_override = os.environ.get("TARUNAI_CONNECT_LLDB_SESSION_FILE")
    if env_override:
        return Path(env_override).expanduser().resolve()

    scope = os.environ.get("TARUNAI_CONNECT_LLDB_SESSION_SCOPE") or os.getcwd()
...
def _spawn_server(state_file: Path):
    cmd = [
        sys.executable,
        "-m",
        "tarunai_connect.lldb.utils.session_server",
        "--state-file",
        str(state_file),
    ]
