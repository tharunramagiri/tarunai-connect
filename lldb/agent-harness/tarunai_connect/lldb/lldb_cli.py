...
import click
from tarunai_connect.lldb.core.session import MEMORY_FIND_MAX_SCAN_SIZE
...
def _get_session():
    global _session
    if _session is None:
        from tarunai_connect.lldb.utils.session_client import RemoteLLDBSessionProxy, resolve_session_file
...
@click.version_option(package_name="tarunai-connect-lldb")
@click.pass_context
def cli(ctx, json_mode, debug, session_file):
    """LLDB CLI - stateful debugger harness with REPL and subcommands."""
    from tarunai_connect.lldb.utils.session_client import resolve_session_file
...
def dap_server(log_file: str | None, profile: str | None):
    ...
    from tarunai_connect.lldb.dap import main as dap_main
...
def repl(ctx):
    ...
    from tarunai_connect.lldb.utils.repl_skin import ReplSkin
