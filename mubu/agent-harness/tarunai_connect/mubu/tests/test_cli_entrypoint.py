...
from tarunai_connect.mubu.mubu_cli import (
    dispatch,
...
def resolve_cli() -> list[str]:
    installed = shutil.which("tarunai-connect-mubu")
    if installed:
        return [installed]
    return [sys.executable, "-m", "tarunai_connect.mubu"]
...
class CliEntrypointTests(unittest.TestCase):
    ...
    def test_dispatch_uses_compat_prog_name_when_requested(self):
        stdout = io.StringIO()
        with contextlib.redirect_stdout(stdout):
            result = dispatch(["--help"], prog_name="tarunai-connect-mubu")
        self.assertEqual(result, 0)
        self.assertIn("Usage: tarunai-connect-mubu", stdout.getvalue())
    ...
    def test_session_state_dir_defaults_to_public_brand_path(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                ...
                mock.patch("tarunai_connect.mubu.mubu_cli.Path.home", return_value=home),
            ):
    ...
    def test_session_state_dir_falls_back_to_legacy_path_when_only_legacy_exists(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            home = Path(tmpdir)
            legacy = home / ".config" / "tarunai-connect-mubu"
    ...
    def test_default_entrypoint_banner_includes_skill_path(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            result = self.run_cli(
                [],
                input_text="exit\n",
                extra_env={"TARUNAI_CONNECT_MUBU_STATE_DIR": tmpdir},
            )
        self.assertEqual(result.returncode, 0, msg=result.stderr)
        self.assertIn("Skill:", result.stdout)
        self.assertIn(
            str(REPO_ROOT / "agent-harness" / "tarunai_connect" / "mubu" / "skills" / "SKILL.md"),
            result.stdout,
        )
    ...
    def test_repl_persists_current_doc_between_processes(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            env = {"TARUNAI_CONNECT_MUBU_STATE_DIR": tmpdir}
