...
class AgentHarnessPackagingTests(unittest.TestCase):
    ...
    def test_agent_harness_contains_canonical_package_tree(self):
        expected_paths = [
            HARNESS_ROOT / "tarunai_connect" / "mubu" / "__init__.py",
            HARNESS_ROOT / "tarunai_connect" / "mubu" / "__main__.py",
            HARNESS_ROOT / "tarunai_connect" / "mubu" / "mubu_cli.py",
            HARNESS_ROOT / "tarunai_connect" / "mubu" / "utils" / "__init__.py",
            HARNESS_ROOT / "tarunai_connect" / "mubu" / "utils" / "repl_skin.py",
            HARNESS_ROOT / "tarunai_connect" / "mubu" / "skills" / "SKILL.md",
            HARNESS_ROOT / "tarunai_connect" / "mubu" / "tests" / "TEST.md",
        ]
    ...
    def test_agent_harness_contains_canonical_test_modules(self):
        expected_paths = [
            HARNESS_ROOT / "tarunai_connect" / "mubu" / "tests" / "test_mubu_probe.py",
            HARNESS_ROOT / "tarunai_connect" / "mubu" / "tests" / "test_cli_entrypoint.py",
            HARNESS_ROOT / "tarunai_connect" / "mubu" / "tests" / "test_agent_harness.py",
            HARNESS_ROOT / "tarunai_connect" / "mubu" / "tests" / "test_core.py",
            HARNESS_ROOT / "tarunai_connect" / "mubu" / "tests" / "test_full_e2e.py",
        ]
        for path in expected_paths:
    ...
    def test_agent_harness_setup_reports_expected_name(self):
        ...
        self.assertEqual(result.stdout.strip(), "tarunai-connect-mubu")
    ...
    def test_repl_skin_matches_tarunai_connect_copy_shape(self):
        repl_skin = (HARNESS_ROOT / "tarunai_connect" / "mubu" / "utils" / "repl_skin.py").read_text()
        self.assertIn('"""tarunai-connect REPL Skin — Unified terminal interface for all CLI harnesses.', repl_skin)
