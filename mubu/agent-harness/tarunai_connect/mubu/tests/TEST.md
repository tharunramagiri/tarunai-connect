...
This file follows the tarunAI Connect habit of keeping the test plan and the executed results in one place.
...
- canonical harness test modules now also exist under `agent-harness/tarunai_connect/mubu/tests/`
...
- canonical source tree exists under `agent-harness/tarunai_connect/mubu/...`
- canonical test modules exist under `agent-harness/tarunai_connect/mubu/tests/...`
...
```bash
TARUNAI_CONNECT_FORCE_INSTALLED=1 python3 -m pytest tarunai_connect/mubu/tests -q
```
...
```bash
python3 -m py_compile mubu_probe.py tarunai_connect/mubu/mubu_cli.py tarunai_connect/mubu/__main__.py
python3 -m py_compile agent-harness/mubu_probe.py agent-harness/tarunai_connect/mubu/mubu_cli.py
python3 -m py_compile agent-harness/tarunai_connect/mubu/__main__.py agent-harness/setup.py
...
python3 -m py_compile agent-harness/tarunai_connect/mubu/tests/__init__.py
python3 -m py_compile agent-harness/tarunai_connect/mubu/tests/test_mubu_probe.py
python3 -m py_compile agent-harness/tarunai_connect/mubu/tests/test_cli_entrypoint.py
python3 -m py_compile agent-harness/tarunai_connect/mubu/tests/test_agent_harness.py
```
...
.venv/bin/tarunai-connect-mubu --help
.venv/bin/tarunai-connect-mubu --json discover daily-current '<daily-folder-ref>'
.venv/bin/tarunai-connect-mubu --json discover daily-current
.venv/bin/tarunai-connect-mubu session status --json
tmpdir=$(mktemp -d)
printf 'exit\n' | env TARUNAI_CONNECT_MUBU_STATE_DIR="$tmpdir" .venv/bin/tarunai-connect-mubu
```
...
unzip -l "$tmpdir"/tarunai_connect_mubu-0.1.1-py3-none-any.whl
```
...
```bash
.venv/bin/tarunai-connect-mubu discover daily-current '<daily-folder-ref>' --json
.venv/bin/tarunai-connect-mubu discover daily-current --json
printf 'exit\n' | env TARUNAI_CONNECT_MUBU_STATE_DIR="$(mktemp -d)" .venv/bin/tarunai-connect-mubu
```
...
- installed REPL banner pointed to `agent-harness/tarunai_connect/mubu/skills/SKILL.md`
...
- built successfully
- wheel contents include `mubu_probe.py`, `tarunai_connect/mubu/README.md`, `tarunai_connect/mubu/skills/SKILL.md`, `tarunai_connect/mubu/tests/TEST.md`, and `tarunai_connect/mubu/utils/repl_skin.py`
...
.venv/bin/python -m pip install -e .
.venv/bin/tarunai-connect-mubu --help
.venv/bin/tarunai-connect-mubu repl --help
tmpdir=$(mktemp -d) && env TARUNAI_CONNECT_MUBU_STATE_DIR="$tmpdir" /usr/bin/zsh -lc "printf 'exit\n' | .venv/bin/tarunai-connect-mubu"
.venv/bin/tarunai-connect-mubu discover daily-current '<daily-folder-ref>' --json
.venv/bin/tarunai-connect-mubu discover daily-current --json
.venv/bin/python -m pip install -e ./agent-harness
...
- `tarunai-connect-mubu --help` renders wrapper + subcommand help
- `tarunai-connect-mubu repl --help` renders REPL help
- no-arg `tarunai-connect-mubu` enters the REPL, exposes app/skill/history banner context, and exits cleanly on `exit`
...
- it is not yet at full tarunAI Connect packaged-harness maturity
