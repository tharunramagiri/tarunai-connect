#!/usr/bin/env python3
r"""
Firefly III CLI - Personal finance management via tarunAI Connect

Firefly III command-line interface based on tarunAI Connect spec,
...
def cli(ctx, use_json, base_url, pat, preset):
    ...
    Based on tarunAI Connect spec, converted from MCP mode to stateless CLI mode,
    ...
    if not base_url or not pat:
        ...
        click.echo("\nUsage:", err=True)
        click.echo("  tarunai-connect-firefly-iii --base-url URL --pat TOKEN", err=True)
