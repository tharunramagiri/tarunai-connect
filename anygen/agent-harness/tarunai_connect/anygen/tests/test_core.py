...
from tarunai_connect.anygen.utils.anygen_backend import (
    get_api_key,
    load_config,
    save_config,
    _make_auth_token,
...
from tarunai_connect.anygen.core.session import Session, HistoryEntry
from tarunai_connect.anygen.core.export import verify_file
...
class TestConfig:
    def test_load_config_missing_file(self, tmp_path):
        with patch("tarunai_connect.anygen.utils.anygen_backend.CONFIG_FILE", tmp_path / "nope.json"):
            assert load_config() == {}

    def test_save_and_load_config(self, tmp_path):
        cfg_file = tmp_path / "cfg" / "config.json"
        with patch("tarunai_connect.anygen.utils.anygen_backend.CONFIG_DIR", tmp_path / "cfg"), \
             patch("tarunai_connect.anygen.utils.anygen_backend.CONFIG_FILE", cfg_file):
...
class TestCreateTask:
    ...
    @patch("tarunai_connect.anygen.utils.anygen_backend.requests.post")
    def test_create_slide_task(self, mock_post):
        ...
        from tarunai_connect.anygen.utils.anygen_backend import create_task
...
class TestQueryTask:
    @patch("tarunai_connect.anygen.utils.anygen_backend.requests.get")
    def test_query_returns_dict(self, mock_get):
        resp = MagicMock()
        resp.status_code = 200
        resp.json.return_value = {"status": "running", "progress": 42}
        mock_get.return_value = resp
        from tarunai_connect.anygen.utils.anygen_backend import query_task
...
class TestPollTask:
    @patch("tarunai_connect.anygen.utils.anygen_backend.time.sleep")
    @patch("tarunai_connect.anygen.utils.anygen_backend.query_task")
    def test_poll_until_completed(self, mock_query, mock_sleep):
        ...
        from tarunai_connect.anygen.utils.anygen_backend import poll_task
    ...
    @patch("tarunai_connect.anygen.utils.anygen_backend.time.time")
    @patch("tarunai_connect.anygen.utils.anygen_backend.time.sleep")
    @patch("tarunai_connect.anygen.utils.anygen_backend.query_task")
