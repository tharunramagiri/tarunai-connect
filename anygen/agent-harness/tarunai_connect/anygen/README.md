...
tarunai-connect-anygen config set api_key "sk-xxx"
...
tarunai-connect-anygen task run --operation slide --prompt "AI trends presentation" --output ./

# Create a task (returns task ID)
tarunai-connect-anygen task create --operation doc --prompt "Technical design document"

# Check task status
tarunai-connect-anygen task status task_xxx

# Poll until completion (with auto-download)
tarunai-connect-anygen task poll task_xxx --output ./

# Download result file
tarunai-connect-anygen task download task_xxx --output ./

# JSON output for agent consumption
tarunai-connect-anygen --json task status task_xxx
```
...
```bash
tarunai-connect-anygen --json task status task_xxx
tarunai-connect-anygen --json task list --limit 5
```
...
python3 -m pytest tarunai_connect/anygen/tests/test_core.py -v

# E2E tests (requires ANYGEN_API_KEY)
ANYGEN_API_KEY=sk-xxx python3 -m pytest tarunai_connect/anygen/tests/test_full_e2e.py -v

# All tests
python3 -m pytest tarunai_connect/anygen/tests/ -v
```
...
tarunai-connect-anygen file upload ./quarterly_data.pdf
# Output: ✓ Uploaded: quarterly_data.pdf → token: tk_abc123

# Requirement analysis (multi-turn)
tarunai-connect-anygen task prepare --message "I need a quarterly review slide deck" --save conv.json
# AnyGen asks clarifying questions...

tarunai-connect-anygen task prepare --message "Focus on revenue growth, 10 slides" --input conv.json --save conv.json
# Status: ready, suggested operation: slide

# Create and download in one step
tarunai-connect-anygen task run \
  --operation slide \
...
# Or step-by-step
tarunai-connect-anygen task create --operation slide --prompt "..."
# Task ID: task_xxx

tarunai-connect-anygen task poll task_xxx --output ./output/
