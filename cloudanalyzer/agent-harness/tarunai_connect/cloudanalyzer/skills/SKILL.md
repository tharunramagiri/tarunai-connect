...
```bash
tarunai-connect-cloudanalyzer [--project FILE] [--json] COMMAND [ARGS]...
```
...
```bash
tarunai-connect-cloudanalyzer evaluate run source.pcd reference.pcd
tarunai-connect-cloudanalyzer --json evaluate run source.pcd reference.pcd
```
...
```bash
tarunai-connect-cloudanalyzer evaluate compare src.pcd tgt.pcd --register gicp
```
...
```bash
tarunai-connect-cloudanalyzer evaluate diff a.pcd b.pcd --threshold 0.1
```
...
```bash
tarunai-connect-cloudanalyzer --json evaluate batch results/ reference.pcd --min-auc 0.95
```
...
```bash
tarunai-connect-cloudanalyzer --json evaluate ground est_ground.pcd est_ng.pcd ref_ground.pcd ref_ng.pcd --min-f1 0.9
```
...
```bash
tarunai-connect-cloudanalyzer evaluate pipeline input.pcd reference.pcd -o output.pcd
```
...
```bash
tarunai-connect-cloudanalyzer --json trajectory evaluate est.csv gt.csv --max-ate 0.5 --max-lateral 0.3
```
...
```bash
tarunai-connect-cloudanalyzer trajectory batch runs/ --reference-dir gt/ --max-drift 1.0
```
...
```bash
tarunai-connect-cloudanalyzer trajectory run-evaluate map.pcd map_ref.pcd traj.csv traj_ref.csv
```
...
```bash
tarunai-connect-cloudanalyzer check init cloudanalyzer.yaml --profile integrated
```
...
```bash
tarunai-connect-cloudanalyzer baseline save qa/summary.json --history-dir qa/history/ --keep 10
```
...
```bash
tarunai-connect-cloudanalyzer --json baseline list --history-dir qa/history/
```
...
```bash
tarunai-connect-cloudanalyzer process downsample cloud.pcd -o down.pcd -v 0.05
```
...
```bash
tarunai-connect-cloudanalyzer process sample cloud.pcd -o sampled.pcd -n 10000
```
...
```bash
tarunai-connect-cloudanalyzer process filter cloud.pcd -o filtered.pcd
```
...
```bash
tarunai-connect-cloudanalyzer process split large.pcd -o tiles/ -g 100
```
...
```bash
tarunai-connect-cloudanalyzer process merge a.pcd b.pcd -o merged.pcd
```
...
```bash
tarunai-connect-cloudanalyzer process convert input.las -o output.pcd
```
...
```bash
tarunai-connect-cloudanalyzer inspect view cloud.pcd
```
...
```bash
tarunai-connect-cloudanalyzer inspect web map.pcd ref.pcd --heatmap
```
...
```bash
tarunai-connect-cloudanalyzer inspect web-export map.pcd ref.pcd -o bundle/
```
...
```bash
tarunai-connect-cloudanalyzer --json info show cloud.pcd
```
...
```bash
tarunai-connect-cloudanalyzer session new -o project.json -n my-run
```
...
```bash
tarunai-connect-cloudanalyzer --project project.json session history --last 20
```
...
```bash
tarunai-connect-cloudanalyzer --json check run cloudanalyzer.yaml --output-json qa/summary.json
...
```bash
tarunai-connect-cloudanalyzer --json evaluate ground \
