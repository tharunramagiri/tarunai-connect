"""Unit tests for tarunai-connect-rms core modules."""
...
class TestBackend:
    ...
    def test_get_api_token_from_env(self):
        from tarunai_connect.rms.utils.rms_backend import get_api_token
    ...
    def test_get_api_token_from_config(self, tmp_path):
        from tarunai_connect.rms.utils import rms_backend
    ...
    def test_require_api_token_missing(self):
        from tarunai_connect.rms.utils.rms_backend import _require_api_token
    ...
    def test_make_auth_headers(self):
        from tarunai_connect.rms.utils.rms_backend import _make_auth_headers

        headers = _make_auth_headers("test-token")
        assert headers == {"Authorization": "Bearer test-token"}

    @patch("tarunai_connect.rms.utils.rms_backend.requests")
    def test_api_get_success(self, mock_requests):
        from tarunai_connect.rms.utils.rms_backend import api_get
    ...
    def test_api_post_success(self, mock_requests):
        from tarunai_connect.rms.utils.rms_backend import api_post
    ...
    def test_api_put_success(self, mock_requests):
        from tarunai_connect.rms.utils.rms_backend import api_put
    ...
    def test_api_delete_success(self, mock_requests):
        from tarunai_connect.rms.utils.rms_backend import api_delete
...
class TestDevices:
    ...
    @patch("tarunai_connect.rms.core.devices.api_get")
    def test_list_devices(self, mock_get):
        from tarunai_connect.rms.core.devices import list_devices
    ...
    def test_get_device(self, mock_get):
        from tarunai_connect.rms.core.devices import get_device
...
class TestCompanies:
    @patch("tarunai_connect.rms.core.companies.api_get")
    def test_list_companies(self, mock_get):
        from tarunai_connect.rms.core.companies import list_companies

        mock_get.return_value = {"success": True, "data": [{"id": 1}]}
        result = list_companies("token")
        assert result["success"] is True

    @patch("tarunai_connect.rms.core.companies.api_post")
    def test_create_company(self, mock_post):
        from tarunai_connect.rms.core.companies import create_company
...
class TestTags:
    @patch("tarunai_connect.rms.core.tags.api_get")
    def test_list_tags(self, mock_get):
        from tarunai_connect.rms.core.tags import list_tags

        mock_get.return_value = {"success": True, "data": [{"id": 1, "name": "office"}]}
        result = list_tags("token")
        assert len(result["data"]) == 1

    @patch("tarunai_connect.rms.core.tags.api_post")
    def test_create_tag(self, mock_post):
        from tarunai_connect.rms.core.tags import create_tag
...
class TestAlerts:
    @patch("tarunai_connect.rms.core.alerts.api_get")
    def test_list_alerts(self, mock_get):
        from tarunai_connect.rms.core.alerts import list_alerts
...
class TestLocation:
    @patch("tarunai_connect.rms.core.location.api_get")
    def test_get_location(self, mock_get):
        from tarunai_connect.rms.core.location import get_location
...
class TestSession:
    def test_session_create(self, tmp_path):
        from tarunai_connect.rms.core.session import Session
...
class TestUsers:
    @patch("tarunai_connect.rms.core.users.api_get")
    def test_list_users(self, mock_get):
        from tarunai_connect.rms.core.users import list_users
    ...
    def test_get_user(self, mock_get):
        from tarunai_connect.rms.core.users import get_user

        mock_get.return_value = {"success": True, "data": {"id": 5, "email": "u@b.com"}}
        result = get_user("token", "5")
        assert result["data"]["id"] == 5

    @patch("tarunai_connect.rms.core.users.api_post")
    def test_invite_user(self, mock_post):
        from tarunai_connect.rms.core.users import invite_user
    ...
    @patch("tarunai_connect.rms.core.users.api_put")
    def test_update_user(self, mock_put):
        from tarunai_connect.rms.core.users import update_user
    ...
    @patch("tarunai_connect.rms.core.users.api_delete")
    def test_delete_user(self, mock_delete):
        from tarunai_connect.rms.core.users import delete_user
...
class TestConfigs:
    @patch("tarunai_connect.rms.core.configs.api_get")
    def test_list_configs(self, mock_get):
        from tarunai_connect.rms.core.configs import list_configs
    ...
    def test_get_config(self, mock_get):
        from tarunai_connect.rms.core.configs import get_config

        mock_get.return_value = {"success": True, "data": {"id": 10, "name": "cfg1"}}
        result = get_config("token", "10")
        assert result["data"]["id"] == 10

    @patch("tarunai_connect.rms.core.configs.api_put")
    def test_update_config(self, mock_put):
        from tarunai_connect.rms.core.configs import update_config
...
class TestRemoteAccess:
    @patch("tarunai_connect.rms.core.remote_access.api_get")
    def test_list_sessions(self, mock_get):
        from tarunai_connect.rms.core.remote_access import list_sessions
    ...
    def test_get_session(self, mock_get):
        from tarunai_connect.rms.core.remote_access import get_session

        mock_get.return_value = {"success": True, "data": {"id": 3, "status": "active"}}
        result = get_session("token", "3")
        assert result["data"]["status"] == "active"

    @patch("tarunai_connect.rms.core.remote_access.api_post")
    def test_create_session(self, mock_post):
        from tarunai_connect.rms.core.remote_access import create_session
    ...
    @patch("tarunai_connect.rms.core.remote_access.api_delete")
    def test_delete_session(self, mock_delete):
        from tarunai_connect.rms.core.remote_access import delete_session
...
class TestLogs:
    @patch("tarunai_connect.rms.core.logs.api_get")
    def test_list_logs(self, mock_get):
        from tarunai_connect.rms.core.logs import list_logs
    ...
    def test_get_log(self, mock_get):
        from tarunai_connect.rms.core.logs import get_log

        mock_get.return_value = {"success": True, "data": {"id": 7, "message": "boot"}}
        result = get_log("token", "7")
        assert result["data"]["id"] == 7

    @patch("tarunai_connect.rms.core.logs.api_delete")
    def test_delete_log(self, mock_delete):
        from tarunai_connect.rms.core.logs import delete_log
...
class TestCredits:
    @patch("tarunai_connect.rms.core.credits.api_get")
    def test_list_credits(self, mock_get):
        from tarunai_connect.rms.core.credits import list_credits
    ...
    @patch("tarunai_connect.rms.core.credits.api_post")
    def test_transfer_credits(self, mock_post):
        from tarunai_connect.rms.core.credits import transfer_credits
    ...
    def test_list_transfer_codes(self, mock_get):
        from tarunai_connect.rms.core.credits import list_transfer_codes
...
class TestFiles:
    @patch("tarunai_connect.rms.core.files.api_get")
    def test_list_files(self, mock_get):
        from tarunai_connect.rms.core.files import list_files
...
class TestReports:
    @patch("tarunai_connect.rms.core.reports.api_get")
    def test_list_reports(self, mock_get):
        from tarunai_connect.rms.core.reports import list_reports
    ...
    def test_get_report(self, mock_get):
        from tarunai_connect.rms.core.reports import get_report

        mock_get.return_value = {"success": True, "data": {"id": 1, "name": "Weekly"}}
        result = get_report("token", "1")
        assert result["data"]["name"] == "Weekly"

    @patch("tarunai_connect.rms.core.reports.api_post")
    def test_create_report(self, mock_post):
        from tarunai_connect.rms.core.reports import create_report
    ...
    @patch("tarunai_connect.rms.core.reports.api_delete")
    def test_delete_report(self, mock_delete):
        from tarunai_connect.rms.core.reports import delete_report
    ...
    def test_list_templates(self, mock_get):
        from tarunai_connect.rms.core.reports import list_templates
    ...
    def test_create_template(self, mock_post):
        from tarunai_connect.rms.core.reports import create_template
...
class TestHotspots:
    @patch("tarunai_connect.rms.core.hotspots.api_get")
    def test_list_hotspots(self, mock_get):
        from tarunai_connect.rms.core.hotspots import list_hotspots
...
class TestPasswords:
    @patch("tarunai_connect.rms.core.passwords.api_get")
    def test_get_password(self, mock_get):
        from tarunai_connect.rms.core.passwords import get_password

        mock_get.return_value = {"success": True, "data": {"device_id": "42", "password": "***"}}
        result = get_password("token", "42")
        assert result["data"]["device_id"] == "42"

    @patch("tarunai_connect.rms.core.passwords.api_put")
    def test_update_password(self, mock_put):
        from tarunai_connect.rms.core.passwords import update_password
...
class TestSmtp:
    @patch("tarunai_connect.rms.core.smtp.api_get")
    def test_list_smtp_configs(self, mock_get):
        from tarunai_connect.rms.core.smtp import list_smtp_configs
    ...
    def test_get_smtp_config(self, mock_get):
        from tarunai_connect.rms.core.smtp import get_smtp_config

        mock_get.return_value = {"success": True, "data": {"id": 1, "host": "smtp.test"}}
        result = get_smtp_config("token", "1")
        assert result["data"]["id"] == 1

    @patch("tarunai_connect.rms.core.smtp.api_post")
    def test_create_smtp_config(self, mock_post):
        from tarunai_connect.rms.core.smtp import create_smtp_config
    ...
    @patch("tarunai_connect.rms.core.smtp.api_put")
    def test_update_smtp_config(self, mock_put):
        from tarunai_connect.rms.core.smtp import update_smtp_config
    ...
    @patch("tarunai_connect.rms.core.smtp.api_delete")
    def test_delete_smtp_config(self, mock_delete):
        from tarunai_connect.rms.core.smtp import delete_smtp_config
...
class TestHandleResponse:
    ...
    def test_success_json(self):
        from tarunai_connect.rms.utils.rms_backend import _handle_response
...
class TestFiles:
    ...
    @patch("tarunai_connect.rms.core.files.requests.post")
    def test_upload_file(self, mock_post):
        from tarunai_connect.rms.core.files import upload_file
...
class TestCLI:
    ...
    def test_root_help(self):
        from tarunai_connect.rms.rms_cli import cli
    ...
    @patch("tarunai_connect.rms.utils.rms_backend.api_get")
