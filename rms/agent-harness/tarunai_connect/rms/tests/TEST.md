# TEST.md — tarunai-connect-rms Test Plan
...
**CLI integration:**
- `test_cli_devices_list` — `tarunai-connect-rms --json devices list` returns valid JSON
- `test_cli_auth_test` — `tarunai-connect-rms auth test` succeeds

### Running Tests

**Important:** Use `python -m pytest` (not bare `pytest`) to avoid namespace package import errors with `tarunai_connect`:
...
python -m pytest tarunai_connect/rms/tests/test_core.py -v

# E2E tests (requires RMS_API_TOKEN)
python -m pytest tarunai_connect/rms/tests/test_full_e2e.py -v
```

Bare `pytest` resolves the local `tarunai_connect/` directory instead of the installed namespace package, causing `ModuleNotFoundError` in tests that use direct imports. Subprocess-based tests (TestCLIIntegrationE2E) are unaffected.
...
| Command | Result |
|---------|--------|
| `python -m tarunai_connect.rms auth test` | "API connection successful" |
| `python -m tarunai_connect.rms devices list` | all devices listed (human-readable) |
| `python -m tarunai_connect.rms --json devices list --limit 5` | Valid JSON, 5 devices, correct metadata |
