#!/usr/bin/env python3
"""End-to-end tests for the ChromaDB tarunAI Connect harness.
...
- tarunai-connect-chromadb installed (pip install -e .)
"""
...
def _resolve_cli():
    """Find the tarunai-connect-chromadb binary.

    Checks in order:
    1. Installed console_script on PATH
    2. python -m tarunai_connect.chromadb fallback
    """
    # Check if the console script is on PATH
    cli_path = shutil.which("tarunai-connect-chromadb")
    if cli_path:
        return [cli_path]
    # Fallback: run as module
    return [sys.executable, "-m", "tarunai_connect.chromadb"]
...
class TestCLIBasic:
    ...
    def test_help_returns_zero(self):
        """tarunai-connect-chromadb --help should exit 0."""
        result = _run(["--help"])
