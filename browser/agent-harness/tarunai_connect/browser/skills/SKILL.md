...
tarunai-connect-browser session daemon-start
...
tarunai-connect-browser session daemon-stop
```
...
1. Start daemon: `tarunai-connect-browser session daemon-start`
2. Run commands: Each command reuses the MCP connection
3. Stop daemon: `tarunai-connect-browser session daemon-stop`
...
- **Allowed schemes**: `http://` and `https://` only (configurable via `TARUNAI_CONNECT_BROWSER_ALLOWED_SCHEMES`)
- **Private network blocking**: Optional via `TARUNAI_CONNECT_BROWSER_BLOCK_PRIVATE=true` (disabled by default)
...
```bash
export TARUNAI_CONNECT_BROWSER_BLOCK_PRIVATE=true
tarunai-connect-browser page open http://localhost:8080  # Will be blocked
```
