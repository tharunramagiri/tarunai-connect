...
_BLOCK_PRIVATE_NETWORKS = os.environ.get("TARUNAI_CONNECT_BROWSER_BLOCK_PRIVATE", "").lower() in ("true", "1")
...
_ALLOWED_SCHEMES = set(
    ...
    for scheme in (
        s.strip().lower()
        for s in os.environ.get("TARUNAI_CONNECT_BROWSER_ALLOWED_SCHEMES", "http,https").split(",")
    )
    if scheme
)
