...
from tarunai_connect.audacity.core.project import create_project, save_project, open_project, get_project_info
from tarunai_connect.audacity.core.tracks import add_track, list_tracks, set_track_property
from tarunai_connect.audacity.core.clips import add_clip, list_clips, split_clip, move_clip, trim_clip
from tarunai_connect.audacity.core.effects import add_effect, list_effects
from tarunai_connect.audacity.core.labels import add_label, list_labels
from tarunai_connect.audacity.core.selection import set_selection, select_all, get_selection
from tarunai_connect.audacity.core.media import probe_audio, check_media, get_duration
from tarunai_connect.audacity.core.export import render_mix, EXPORT_PRESETS
from tarunai_connect.audacity.core.session import Session
from tarunai_connect.audacity.utils.audio_utils import (
...
def _resolve_cli(name):
    ...
    module = name.replace("tarunai-connect-", "tarunai_connect.") + "." + name.split("-")[-1] + "_cli"
    print(f"[_resolve_cli] Falling back to: {sys.executable} -m {module}")
    return [sys.executable, "-m", module]


class TestCLISubprocess:
    CLI_BASE = _resolve_cli("tarunai-connect-audacity")
...
class TestSoXBackend:
    ...
    def test_sox_is_installed(self):
        from tarunai_connect.audacity.utils.sox_backend import find_sox
        path = find_sox()
        assert os.path.exists(path)
        print(f"\n  SoX binary: {path}")

    def test_sox_version(self):
        from tarunai_connect.audacity.utils.sox_backend import get_version
        version = get_version()
...
class TestSoXAudioE2E:
    ...
    def test_generate_sine_tone_wav(self):
        ...
        from tarunai_connect.audacity.utils.sox_backend import generate_tone
    ...
    def test_apply_reverb_effect(self):
        ...
        from tarunai_connect.audacity.utils.sox_backend import generate_tone, apply_effect
    ...
    def test_convert_sample_rate(self):
        ...
        from tarunai_connect.audacity.utils.sox_backend import generate_tone, convert_format
