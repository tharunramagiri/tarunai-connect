...
tarunai-connect-unrealinsights --json backend info

# Inspect the local Trace Store and select the latest trace
tarunai-connect-unrealinsights --json store info
tarunai-connect-unrealinsights --json store latest --set-current

# Find or build UnrealInsights.exe for a custom engine root
tarunai-connect-unrealinsights --json backend ensure-insights `
  --engine-root 'D:\code\D5\d5render-ue5_3'

# Bind a trace file for the current session
tarunai-connect-unrealinsights trace set D:\captures\session.utrace

# Start a background capture session and keep it running
tarunai-connect-unrealinsights --json capture start `
...
tarunai-connect-unrealinsights --json capture start --replace `
  --project 'D:\Projects\MyGame\MyGame.uproject' `
  --engine-root 'D:\Program Files\Epic Games\UE_5.5' `
  --output-trace D:\captures\replacement_session.utrace

# Check current capture status
tarunai-connect-unrealinsights --json capture status

# Create a best-effort snapshot without ending the session
tarunai-connect-unrealinsights --json capture snapshot D:\captures\live_snapshot.utrace

# Stop the tracked capture session
tarunai-connect-unrealinsights --json capture stop
...
tarunai-connect-unrealinsights --json live processes
tarunai-connect-unrealinsights --json live trace-status --pid 1234

# Open Unreal Insights GUI and keep it running
tarunai-connect-unrealinsights --json gui open-latest

# Export threads
tarunai-connect-unrealinsights --json -t D:\captures\session.utrace export threads D:\out\threads.csv

# Export timer statistics for a region
tarunai-connect-unrealinsights --json -t D:\captures\session.utrace export timer-stats `
  D:\out\timer_stats.csv --threads=GameThread --timers=* --region=EXPORT_CAPTURE

# Execute a response file
tarunai-connect-unrealinsights --json -t D:\captures\session.utrace batch run-rsp D:\out\export.rsp

# Export and summarize timing/counter hotspots
tarunai-connect-unrealinsights --json -t D:\captures\session.utrace analyze summary --out D:\out\summary

# Launch a traced UE target and wait for completion
tarunai-connect-unrealinsights --json capture run `
  --project 'D:\Projects\MyGame\MyGame.uproject' `
  --engine-root 'D:\Program Files\Epic Games\UE_5.5' `
...
tarunai-connect-unrealinsights capture run `
  --project 'D:\Projects\MyGame\MyGame.uproject' `
  --engine-root 'D:\Program Files\Epic Games\UE_5.5'
...
```powershell
tarunai-connect-unrealinsights --json capture start --project ... --engine-root ...
tarunai-connect-unrealinsights --json capture status
tarunai-connect-unrealinsights --json capture snapshot
tarunai-connect-unrealinsights --json capture stop
```
...
```powershell
tarunai-connect-unrealinsights --json store info
tarunai-connect-unrealinsights --json store list --live-only
tarunai-connect-unrealinsights --json store latest --set-current
```
...
$env:UNREALINSIGHTS_LIVE_EXEC='ushell-wrapper --pid {pid} --cmd "{cmd}"'
tarunai-connect-unrealinsights --json live processes
tarunai-connect-unrealinsights --json live bookmark --pid 1234 "BeforeExport"
tarunai-connect-unrealinsights --json live snapshot --pid 1234 D:\captures\snapshot.utrace
tarunai-connect-unrealinsights --json live stop-trace --pid 1234
```
...
```powershell
tarunai-connect-unrealinsights --json gui status
tarunai-connect-unrealinsights --json gui open --trace D:\captures\session.utrace
tarunai-connect-unrealinsights --json gui open-latest
```
...
```powershell
tarunai-connect-unrealinsights --json analyze summary --skip-export --out D:\out\summary
```
...
```bash
cd unrealinsights/agent-harness
pytest tarunai_connect/unrealinsights/tests/test_core.py -v
pytest tarunai_connect/unrealinsights/tests/test_full_e2e.py -v -s
```
