#!/usr/bin/env python3
"""tarunai-connect-iterm2 — Stateful CLI harness for iTerm2.
...
Usage:
    # One-shot commands
    tarunai-connect-iterm2 app status
    tarunai-connect-iterm2 window list
    tarunai-connect-iterm2 window create --profile "Default"
    tarunai-connect-iterm2 session send --session-id <id> "echo hello\\n"
...
from tarunai_connect.iterm2_ctl.core import (
...
from tarunai_connect.iterm2_ctl.utils.iterm2_backend import run_iterm2
...
def cli(ctx, use_json):
    """tarunai-connect-iterm2 — Control iTerm2 from the command line.
...
def app_alert(title, subtitle, buttons, window_id):
    ...
    \b
      tarunai-connect-iterm2 app alert "Deploy?" "Push to production?"
      tarunai-connect-iterm2 app alert "Choose" "Pick one" --button Yes --button No
    """
...
def app_text_input(title, subtitle, placeholder, default_value, window_id):
    ...
    \b
      tarunai-connect-iterm2 app text-input "Rename" "Enter new name:" --default "myapp"
    """
    wid = window_id or get_state().window_id
...
def app_file_panel(title, path, extensions, dirs, multi):
    ...
    \b
      tarunai-connect-iterm2 app file-panel
      tarunai-connect-iterm2 app file-panel --path ~/Documents --ext py --ext txt
      tarunai-connect-iterm2 app file-panel --dirs --multi
    """
...
def app_save_panel(title, path, filename):
    ...
    \b
      tarunai-connect-iterm2 app save-panel
      tarunai-connect-iterm2 app save-panel --path ~/Desktop --filename output.txt
    """
...
def tab_select_pane(direction, tab_id):
    ...
    \b
      tarunai-connect-iterm2 tab select-pane right
      tarunai-connect-iterm2 tab select-pane below --tab-id <id>
    """
    tid = tab_id or get_state().tab_id
    if not tid:
        raise click.UsageError("No tab ID specified.")
...
def session_send(text, session_id, no_newline, suppress_broadcast):
    ...
    Example:
        tarunai-connect-iterm2 session send "ls -la"
        tarunai-connect-iterm2 session send "pwd" --session-id w0t0p0
    """
...
def session_scrollback(session_id, lines, tail, strip):
    ...
    \b
      tarunai-connect-iterm2 session scrollback                  # all history
      tarunai-connect-iterm2 session scrollback --tail 100       # last 100 lines
      tarunai-connect-iterm2 session scrollback --lines 500      # first 500 lines
    """
    sid = session_id or get_state().session_id
    if not sid:
        raise click.UsageError("No session ID specified.")
...
def session_run_tmux_cmd(command, session_id):
    ...
    Example:
        ...
        tarunai-connect-iterm2 session run-tmux-cmd "list-sessions"
    """
    sid = session_id or get_state().session_id
    if not sid:
        raise click.UsageError("No session ID specified.")
...
def session_inject(data, session_id, use_hex):
    ...
    \b
      tarunai-connect-iterm2 session inject $'\\x1b[2J'      # clear screen (ESC[2J)
      tarunai-connect-iterm2 session inject "1b5b324a" --hex  # same in hex
      tarunai-connect-iterm2 session inject $'\\x07'          # bell
    """
    sid = session_id or get_state().session_id
    if not sid:
        raise click.UsageError("No session ID specified.")
...
def profile_get(guid):
    ...
    \b
      tarunai-connect-iterm2 profile list          # find the GUID
      tarunai-connect-iterm2 profile get <guid>    # get details
    """
...
def tmux_set_visible(tmux_window_id, mode, connection_id):
    ...
    \b
      tarunai-connect-iterm2 tmux set-visible @1 off   # hide
      tarunai-connect-iterm2 tmux set-visible @1 on    # show
    """
    visible = mode == "on"
...
def tmux_bootstrap(attach, session_id, timeout):
    ...
    \b
      tarunai-connect-iterm2 tmux bootstrap                # start new session
      tarunai-connect-iterm2 tmux bootstrap --attach       # attach to existing
      tarunai-connect-iterm2 tmux bootstrap --session-id w0t0p0
    """
    sid = session_id or get_state().session_id
...
def broadcast_set(groups):
    ...
    \b
      tarunai-connect-iterm2 broadcast set "s1,s2" "s3,s4"
    """
...
def broadcast_add(session_ids):
    ...
    \b
      tarunai-connect-iterm2 broadcast add s1 s2
    """
...
def pref_list_keys(name_filter):
    ...
    \b
      tarunai-connect-iterm2 pref list-keys
      tarunai-connect-iterm2 pref list-keys --filter tmux
      tarunai-connect-iterm2 pref list-keys --filter font
    """
...
def repl(ctx):
    ...
    from tarunai_connect.iterm2_ctl.utils.repl_skin import ReplSkin
