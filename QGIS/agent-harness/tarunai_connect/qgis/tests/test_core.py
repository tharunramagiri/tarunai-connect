"""Core tests for tarunai-connect-qgis using PyQGIS and focused backend mocks."""
...
from tarunai_connect.qgis.utils import qgis_backend as backend
...
def test_find_qgis_process_missing_raises_clear_error():
    with mock.patch("tarunai_connect.qgis.utils.qgis_backend.shutil.which", return_value=None):
...
def test_run_process_json_normalizes_backend_failure():
    ...
    with mock.patch("tarunai_connect.qgis.utils.qgis_backend.find_qgis_process", return_value="/usr/bin/qgis_process"):
        with mock.patch("tarunai_connect.qgis.utils.qgis_backend.subprocess.run", return_value=completed):
