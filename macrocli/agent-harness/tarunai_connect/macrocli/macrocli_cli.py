...
Usage (one-shot):
    tarunai-connect-macrocli macro run export_file --param output=/tmp/out.png --json
    tarunai-connect-macrocli macro list --json
    tarunai-connect-macrocli macro info export_file --json

Usage (REPL):
    tarunai-connect-macrocli          # enters interactive REPL
    tarunai-connect-macrocli repl
"""
...
from tarunai_connect.macrocli.core.registry import MacroRegistry
from tarunai_connect.macrocli.core.runtime import MacroRuntime
from tarunai_connect.macrocli.core.session import ExecutionSession
...
def cli(ctx, json_flag, dry_run_flag, session_id):
    ...
    \b
    Quick start:
      tarunai-connect-macrocli macro list
      tarunai-connect-macrocli macro info <name>
      tarunai-connect-macrocli macro run <name> --param key=value
...
def macro_run(name, param, macro_file):
    ...
    if macro_file:
        ...
        from tarunai_connect.macrocli.core.macro_model import load_from_yaml
        from tarunai_connect.macrocli.core.routing import RoutingEngine
        from tarunai_connect.macrocli.core.runtime import MacroRuntime, ExecutionResult
        from tarunai_connect.macrocli.core.registry import MacroRegistry
        try:
...
def macro_record(name, output_dir, timeout, do_agent_review,
    ...
    """
    try:
        from tarunai_connect.macrocli.core.recorder import MacroRecorder
    ...
    if do_auto_param and type_steps:
        try:
            from tarunai_connect.macrocli.core.parameterize import (
    ...
    if do_parameterize and type_steps:
        from tarunai_connect.macrocli.core.parameterize import interactive_parameterize
...
def macro_assist(name, goal, screenshot, output, api_key, model):
    ...
    """
    try:
        from tarunai_connect.macrocli.core.llm_assist import generate_macro
    except ImportError as e:
        click.echo(f"Error: {e}", err=True)
        sys.exit(1)
...
def macro_capture_template(output_path, x, y, width, height):
    ...
    """
    try:
        from tarunai_connect.macrocli.backends.visual_anchor import VisualAnchorBackend
        from tarunai_connect.macrocli.backends.base import BackendContext
        from tarunai_connect.macrocli.core.macro_model import MacroStep
    except ImportError as e:
        click.echo(f"Error: {e}", err=True)
        sys.exit(1)
...
def repl(ctx):
    ...
    from tarunai_connect.macrocli.utils.repl_skin import ReplSkin
