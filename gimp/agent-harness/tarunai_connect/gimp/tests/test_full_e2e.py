...
from tarunai_connect.gimp.core.project import create_project, save_project, open_project, get_project_info
from tarunai_connect.gimp.core.layers import add_layer, add_from_file, list_layers, remove_layer
from tarunai_connect.gimp.core.filters import add_filter, list_filters
from tarunai_connect.gimp.core.canvas import resize_canvas, scale_canvas, crop_canvas, set_mode
from tarunai_connect.gimp.core.media import probe_image, check_media
from tarunai_connect.gimp.core.export import render
from tarunai_connect.gimp.core.session import Session
...
class TestCLISubprocess:
    CLI_BASE = _resolve_cli("tarunai-connect-gimp")
...
class TestGIMPBackend:
    ...
    def test_gimp_is_installed(self):
        from tarunai_connect.gimp.utils.gimp_backend import find_gimp
        path = find_gimp()
        assert os.path.exists(path)
        print(f"\n  GIMP binary: {path}")

    def test_gimp_version(self):
        from tarunai_connect.gimp.utils.gimp_backend import get_version
        version = get_version()
...
class TestGIMPRenderE2E:
    ...
    def test_create_and_export_png(self):
        ...
        from tarunai_connect.gimp.utils.gimp_backend import create_and_export
