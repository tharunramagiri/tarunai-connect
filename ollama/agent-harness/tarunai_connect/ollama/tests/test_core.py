"""Unit tests for tarunai-connect-ollama — no Ollama server required."""
...
from tarunai_connect.ollama.ollama_cli import cli, _format_size
from tarunai_connect.ollama.utils.ollama_backend import DEFAULT_BASE_URL
...
class TestBackend:
    ...
    @patch("tarunai_connect.ollama.utils.ollama_backend.requests.get")
    def test_is_available_true(self, mock_get):
        from tarunai_connect.ollama.utils.ollama_backend import is_available
        mock_resp = MagicMock()
    ...
    def test_api_get_connection_error(self, mock_get):
        from tarunai_connect.ollama.utils.ollama_backend import api_get
        import requests
        mock_get.side_effect = requests.exceptions.ConnectionError()
        with pytest.raises(RuntimeError, match="Cannot connect to Ollama"):
            api_get("http://localhost:11434", "/api/tags")

    @patch("tarunai_connect.ollama.utils.ollama_backend.requests.post")
    def test_api_post_connection_error(self, mock_post):
        from tarunai_connect.ollama.utils.ollama_backend import api_post
        import requests
        mock_post.side_effect = requests.exceptions.ConnectionError()
        with pytest.raises(RuntimeError, match="Cannot connect to Ollama"):
            api_post("http://localhost:11434", "/api/show", {"name": "test"})

    @patch("tarunai_connect.ollama.utils.ollama_backend.requests.delete")
    def test_api_delete_connection_error(self, mock_delete):
        from tarunai_connect.ollama.utils.ollama_backend import api_delete
        import requests
...
class TestErrorHandling:
    @patch("tarunai_connect.ollama.core.models.api_get")
    ...
    @patch("tarunai_connect.ollama.core.server.api_get")
...
class TestModelCommands:
    ...
    @patch("tarunai_connect.ollama.core.models.api_post")
    ...
    @patch("tarunai_connect.ollama.core.models.api_delete")
...
class TestEmbedCommands:
    @patch("tarunai_connect.ollama.core.embeddings.api_post")
...
class TestGenerateTextCommands:
    @patch("tarunai_connect.ollama.core.generate.api_post")
    ...
    @patch("tarunai_connect.ollama.core.generate.api_post_stream")
...
class TestModelPullCommands:
    ...
    @patch("tarunai_connect.ollama.core.models.api_post_stream")
...
class TestBackendStreaming:
    ...
    def test_api_post_stream_success(self, mock_post):
        from tarunai_connect.ollama.utils.ollama_backend import api_post_stream
        mock_resp = MagicMock()
        mock_resp.status_code = 200
        mock_resp.raise_for_status.return_value = None
...
class TestCoreModules:
    ...
    def test_generate_builds_correct_payload(self, mock_api):
        from tarunai_connect.ollama.core.generate import generate
    ...
    def test_chat_builds_correct_payload(self, mock_api):
        from tarunai_connect.ollama.core.generate import chat
    ...
    def test_embed_builds_correct_payload(self, mock_api):
        from tarunai_connect.ollama.core.embeddings import embed
    ...
    def test_copy_model_payload(self, mock_api):
        from tarunai_connect.ollama.core.models import copy_model
        mock_api.return_value = {"status": "ok"}
    ...
    def test_delete_model_payload(self, mock_api):
        from tarunai_connect.ollama.core.models import delete_model
        mock_api.return_value = {"status": "ok"}
...
class TestStreamToStdout:
    def test_stream_to_stdout_generate(self, capsys):
        from tarunai_connect.ollama.core.generate import stream_to_stdout
        chunks = iter([
            {"response": "Hello", "done": False},
