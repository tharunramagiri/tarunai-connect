...
This CLI is installed as part of the tarunai-connect-shotcut package:

```bash
pip install tarunai-connect-shotcut
```
...
```bash
# Show help
tarunai-connect-shotcut --help

# Start interactive REPL mode
tarunai-connect-shotcut

# Create a new project
tarunai-connect-shotcut project new -o project.json

# Run with JSON output (for agent consumption)
tarunai-connect-shotcut --json project info -p project.json
```
...
tarunai-connect-shotcut
# or with a project:
tarunai-connect-shotcut --project my_project.mlt
```
...
```bash
tarunai-connect-shotcut --json --project edit.mlt preview live start --recipe quick --mode poll --source-poll-ms 500
```
...
```bash
tarunai-connect-shotcut project new -o myproject.json
# Or with JSON output for programmatic use
tarunai-connect-shotcut --json project new -o myproject.json
```
...
```bash
tarunai-connect-shotcut --project myproject.json export render output.mp4 --overwrite
```
...
```bash
tarunai-connect-shotcut --project myproject.mlt media import intro.mp4
tarunai-connect-shotcut --project myproject.mlt timeline add-clip clip0 \
  --track 1 --in 00:00:00.000 --out 00:00:04.000 --at 00:00:00.000

tarunai-connect-shotcut --project myproject.mlt media import broll.mp4
tarunai-connect-shotcut --project myproject.mlt timeline add-clip clip1 \
...
```bash
tarunai-connect-shotcut --project myproject.mlt filter volume-envelope \
  --track 2 \
  --point 00:00:00.000=1.0 \
  --point 00:00:03.000=0.35 \
  --point 00:00:05.000=1.0

tarunai-connect-shotcut --project myproject.mlt filter duck \
  --track 2 \
...
```bash
# Human output
tarunai-connect-shotcut project info -p project.json
