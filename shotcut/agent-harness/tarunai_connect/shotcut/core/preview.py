    ...
    *,
    ...
    manifest = finalize_bundle(
        ...
        generator={
            ...
            "harness_version": HARNESS_VERSION,
            "command": command or f"tarunai-connect-shotcut preview capture --recipe {recipe}",
        },
...
def _preview_base_dir(project_path: Optional[str] = None, root_dir: Optional[str] = None) -> Path:
    ...
    if project_path:
        return Path(project_path).expanduser().resolve().parent / ".tarunai-connect" / "previews" / "shotcut"
    return Path.home() / ".tarunai-connect" / "previews" / "shotcut"
...
def _publish_live_session(
    ...
    *,
    ...
    payload = {
        ...
        "poller": poller,
        "publish_command": (
            f"tarunai-connect-shotcut{project_flag} preview live push --recipe {recipe}{root_flag}"
        ).strip(),
        "watch_command": (
        ...
        "start_command": (
            f"tarunai-connect-shotcut{project_flag} preview live start --recipe {recipe} "
            f"--mode {current_live_mode} --source-poll-ms {current_source_poll_ms}{root_flag}"
        ).strip(),
        "monitor_command": (
            f"tarunai-connect-shotcut preview live monitor --session-dir {session_dir}"
        ),
    }
    _write_json(session_dir / "session.json", payload)
    return _with_live_refs(session_dir, payload)
...
def poll_live_session_once(session_dir: str) -> Dict[str, Any]:
    ...
    try:
        ...
        live_payload = live_start(
            ...
            command=str(payload.get("monitor_command") or f"tarunai-connect-shotcut preview live monitor --session-dir {session_path}"),
