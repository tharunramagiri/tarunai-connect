...
from tarunai_connect.shotcut.core.session import Session
from tarunai_connect.shotcut.core import project as proj_mod
from tarunai_connect.shotcut.core import timeline as tl_mod
from tarunai_connect.shotcut.core import filters as filt_mod
from tarunai_connect.shotcut.core import media as media_mod
from tarunai_connect.shotcut.core import export as export_mod
from tarunai_connect.shotcut.core import transitions as trans_mod
from tarunai_connect.shotcut.core import compositing as comp_mod
from tarunai_connect.shotcut.core import preview as preview_mod
...
def _spawn_live_poller(session_dir: str) -> dict:
    ...
    cli_path = shutil.which("tarunai-connect-shotcut")
    if cli_path:
        ...
        command = [
            sys.executable,
            "-m",
            "tarunai_connect.shotcut.shotcut_cli",
            "preview",
            "live",
            "monitor",
            "--session-dir",
...
def project_xml():
    ...
    from tarunai_connect.shotcut.utils.mlt_xml import mlt_to_string
...
def preview_capture(recipe, force, root_dir):
    ...
    result = preview_mod.capture(
        ...
        force=force,
        root_dir=root_dir,
        command=f"tarunai-connect-shotcut --project {session_obj.project_path or ''} preview capture --recipe {recipe}".strip(),
    )
...
def preview_live_start(recipe, force, root_dir, poll_ms, mode, source_poll_ms, open_window):
    ...
    result = preview_mod.live_start(
        ...
        command=(
            f"tarunai-connect-shotcut --project {session_obj.project_path or ''} "
...
def repl(project_path):
    ...
    from tarunai_connect.shotcut.utils.repl_skin import ReplSkin
