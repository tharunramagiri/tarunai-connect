...
Usage:
    # One-shot commands
    tarunai-connect-minimax chat --prompt "Hello" --model MiniMax-M2.7
    tarunai-connect-minimax stream --prompt "Tell me a story"
...
from tarunai_connect.minimax.core.session import ChatSession
from tarunai_connect.minimax.utils.minimax_backend import (
    get_api_key,
    load_config,
    save_config,
    chat_completion,
...
def get_session():
    global _session
    if _session is None:
        sf = str(Path.home() / ".tarunai-connect-minimax" / "session.json")
...
def config_path():
    ...
    from tarunai_connect.minimax.utils.minimax_backend import CONFIG_FILE
...
def repl():
    ...
    from tarunai_connect.minimax.utils.repl_skin import ReplSkin
