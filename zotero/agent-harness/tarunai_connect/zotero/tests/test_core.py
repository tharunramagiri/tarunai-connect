...
from tarunai_connect.zotero.core import analysis, catalog, discovery, experimental, imports as imports_mod, notes as notes_mod, rendering, session as session_mod
from tarunai_connect.zotero.tests._helpers import create_sample_environment, fake_zotero_http_server, sample_pdf_bytes
from tarunai_connect.zotero.utils import openai_api, zotero_http, zotero_paths, zotero_sqlite
...
class PathDiscoveryTests(unittest.TestCase):
    ...
    def test_build_environment_falls_back_to_home_zotero(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with mock.patch("tarunai_connect.zotero.utils.zotero_paths.Path.home", return_value=home):
    ...
    def test_find_executable_returns_none_when_unresolved(self):
        with mock.patch.dict("os.environ", {}, clear=True):
            with mock.patch("tarunai_connect.zotero.utils.zotero_paths.shutil.which", return_value=None):
...
class SessionTests(unittest.TestCase):
    def test_save_and_load_session_state(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            with mock.patch.dict("os.environ", {"TARUNAI_CONNECT_ZOTERO_STATE_DIR": tmpdir}, clear=False):
...
class ImportCoreTests(unittest.TestCase):
    ...
    def test_import_json_uses_session_collection_and_tags(self):
        ...
        with mock.patch.object(self.runtime, "connector_available", True):
            with mock.patch("tarunai_connect.zotero.utils.zotero_http.connector_save_items") as save_items:
                with mock.patch("tarunai_connect.zotero.utils.zotero_http.connector_update_session") as update_session:
        ...
        self.assertTrue(submitted_items[0]["id"].startswith("tarunai-connect-zotero-"))
    ...
    def test_import_file_posts_raw_text_and_explicit_tree_view_target(self):
        ...
        with mock.patch.object(self.runtime, "connector_available", True):
            with mock.patch("tarunai_connect.zotero.utils.zotero_http.connector_import_text", return_value=[{"title": "Imported Title"}]) as import_text:
                with mock.patch("tarunai_connect.zotero.utils.zotero_http.connector_update_session") as update_session:
    ...
    def test_import_json_strips_inline_attachments_and_uploads_local_pdf(self):
        ...
        with mock.patch.object(self.runtime, "connector_available", True):
            with mock.patch("tarunai_connect.zotero.utils.zotero_http.connector_save_items") as save_items:
                with mock.patch("tarunai_connect.zotero.utils.zotero_http.connector_update_session"):
                    with mock.patch("tarunai_connect.zotero.utils.zotero_http.connector_save_attachment") as save_attachment:
                        payload = imports_mod.import_json(
                            self.runtime,
                            json_path,
    ...
    def test_import_json_url_attachment_uses_delay_and_default_timeout(self):
        ...
        with fake_zotero_http_server() as server:
            ...
            with mock.patch.object(self.runtime, "connector_available", True):
                with mock.patch("tarunai_connect.zotero.utils.zotero_http.connector_save_items"):
                    with mock.patch("tarunai_connect.zotero.utils.zotero_http.connector_update_session"):
                        with mock.patch("tarunai_connect.zotero.utils.zotero_http.connector_save_attachment") as save_attachment:
                            with mock.patch("tarunai_connect.zotero.core.imports.time.sleep") as sleep:
                                payload = imports_mod.import_json(
                                    self.runtime,
                                    json_path,
    ...
    def test_import_file_manifest_partial_success_records_attachment_failures(self):
        ...
        with mock.patch.object(self.runtime, "connector_available", True):
            with mock.patch(
                "tarunai_connect.zotero.utils.zotero_http.connector_import_text",
...
class WorkflowCoreTests(unittest.TestCase):
    ...
    def test_collection_scoped_item_find_prefers_local_api(self):
        with mock.patch.object(self.runtime, "local_api_available", True):
            with mock.patch("tarunai_connect.zotero.utils.zotero_http.local_api_get_json", return_value=[{"key": "REG12345"}]) as local_api:
    ...
    def test_group_library_local_api_scope_and_search_routes(self):
        ...
        with mock.patch.object(self.runtime, "local_api_available", True):
            with mock.patch("tarunai_connect.zotero.utils.zotero_http.local_api_get_json", return_value=[{"key": "GROUPKEY"}]) as local_api:
    ...
    def test_note_add_builds_child_note_payload(self):
        with mock.patch.object(self.runtime, "connector_available", True):
            with mock.patch("tarunai_connect.zotero.utils.zotero_http.get_selected_collection", return_value={"libraryID": 1}):
                with mock.patch("tarunai_connect.zotero.utils.zotero_http.connector_save_items") as save_items:
    ...
    def test_item_context_aggregates_exports_and_links(self):
        with mock.patch.object(self.runtime, "local_api_available", True):
            with mock.patch("tarunai_connect.zotero.core.rendering.export_item", side_effect=[{"content": "@article{sample}"}, {"content": '{"id":"sample"}'}]):
    ...
    def test_item_analyze_requires_api_key_and_uses_openai(self):
        ...
        with mock.patch.dict("os.environ", {"OPENAI_API_KEY": "test-key"}, clear=False):
            with mock.patch("tarunai_connect.zotero.core.analysis.build_item_context", return_value={"item": {"key": "REG12345"}, "prompt_context": "Title: Sample"}):
                with mock.patch("tarunai_connect.zotero.utils.openai_api.create_text_response", return_value={"response_id": "resp_123", "answer": "Analysis", "raw": {}}) as create_response:
    ...
    def test_rendering_uses_group_library_local_api_scope(self):
        with mock.patch.object(self.runtime, "local_api_available", True):
            with mock.patch("tarunai_connect.zotero.utils.zotero_http.local_api_get_text", return_value="TY  - JOUR\nER  - \n") as get_text:
