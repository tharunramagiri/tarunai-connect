...
from tarunai_connect.zotero.tests._helpers import create_sample_environment, fake_zotero_http_server, sample_pdf_bytes
from tarunai_connect.zotero.core import session as session_mod
from tarunai_connect.zotero.zotero_cli import RootCliConfig, _handle_repl_builtin, dispatch, repl_help_text, run_repl
...
def resolve_cli() -> list[str]:
    force_installed = os.environ.get("TARUNAI_CONNECT_FORCE_INSTALLED", "").strip() == "1"
    installed = shutil.which("tarunai-connect-zotero")
    if installed:
        return [installed]
    scripts_dir = Path(sysconfig.get_path("scripts"))
    for candidate in (scripts_dir / "tarunai-connect-zotero.exe", scripts_dir / "tarunai-connect-zotero"):
        if candidate.exists():
            return [str(candidate)]
    if force_installed:
        raise RuntimeError("tarunai-connect-zotero not found in PATH. Install it with: py -m pip install -e .")
    return [sys.executable, "-m", "tarunai_connect.zotero"]
...
class CliEntrypointTests(unittest.TestCase):
    ...
    def run_cli(self, args, input_text=None, extra_env=None):
        ...
        env["TARUNAI_CONNECT_ZOTERO_STATE_DIR"] = str(Path(self.tmpdir.name) / "state")
        if extra_env:
    ...
    def test_dispatch_uses_requested_prog_name(self):
        result = dispatch(["--help"], prog_name="tarunai-connect-zotero")
        self.assertEqual(result, 0)

    def test_force_installed_mode_requires_real_command(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            with mock.patch.dict("os.environ", {"TARUNAI_CONNECT_FORCE_INSTALLED": "1"}, clear=False):
    ...
    def test_default_entrypoint_starts_repl(self):
        ...
        self.assertEqual(result.returncode, 0, msg=result.stderr)
        self.assertIn("tarunai-connect-zotero", result.stdout)
    ...
    def test_repl_builtin_use_library_uses_root_runtime_config(self):
        ...
        with mock.patch("tarunai_connect.zotero.zotero_cli.current_session", return_value=state):
            with mock.patch("tarunai_connect.zotero.zotero_cli.session_mod.save_session_state"):
                with mock.patch("tarunai_connect.zotero.zotero_cli.session_mod.append_command_history"):
                    with mock.patch("tarunai_connect.zotero.zotero_cli.discovery.build_runtime_context", return_value=object()) as build_runtime:
                        with mock.patch("tarunai_connect.zotero.zotero_cli._normalize_session_library", return_value=2):
    ...
    def test_repl_builtin_use_selected_uses_root_runtime_config(self):
        ...
        with mock.patch("tarunai_connect.zotero.zotero_cli.current_session", return_value=state):
            with mock.patch("tarunai_connect.zotero.zotero_cli.discovery.build_runtime_context", return_value=runtime) as build_runtime:
                with mock.patch("tarunai_connect.zotero.zotero_cli.catalog.use_selected_collection", return_value=selected) as use_selected:
                    with mock.patch("tarunai_connect.zotero.zotero_cli._persist_selected_collection", return_value=state):
                        with mock.patch("tarunai_connect.zotero.zotero_cli.session_mod.append_command_history"):
    ...
    def test_run_repl_dispatches_commands_with_root_flags(self):
        ...
        with mock.patch("tarunai_connect.zotero.zotero_cli.ReplSkin.create_prompt_session", return_value=None):
            with mock.patch("tarunai_connect.zotero.zotero_cli._safe_print_banner"), mock.patch(
                "tarunai_connect.zotero.zotero_cli._safe_print_goodbye"
            ):
                with mock.patch("builtins.input", side_effect=["item get REG12345", "exit"]):
                    with mock.patch("tarunai_connect.zotero.zotero_cli.current_session", return_value=session_mod.default_session_state()):
                        with mock.patch(
                            "tarunai_connect.zotero.zotero_cli.session_mod.expand_repl_aliases_with_state",
                            return_value=["item", "get", "REG12345"],
                        ):
                            with mock.patch("tarunai_connect.zotero.zotero_cli.dispatch", return_value=0) as dispatch_mock:
        ...
        with fake_zotero_http_server() as server:
            analyze_result = self.run_cli(
                ...
                extra_env={
                    ...
                    "TARUNAI_CONNECT_ZOTERO_OPENAI_URL": f"http://127.0.0.1:{server['port']}/v1/responses",
                },
            )
