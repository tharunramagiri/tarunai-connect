...
from tarunai_connect.zotero.core import discovery
from tarunai_connect.zotero.tests._helpers import sample_pdf_bytes
from tarunai_connect.zotero.utils import zotero_paths, zotero_sqlite
...
class ZoteroFullE2E(unittest.TestCase):
    ...
    @unittest.skipUnless(os.environ.get("TARUNAI_CONNECT_ZOTERO_ENABLE_WRITE_E2E") == "1", "Write E2E disabled")
    def test_opt_in_write_import_commands(self):
        target = os.environ.get("TARUNAI_CONNECT_ZOTERO_IMPORT_TARGET", "").strip()
        self.assertTrue(target, "TARUNAI_CONNECT_ZOTERO_IMPORT_TARGET must be set when write E2E is enabled")

        with tempfile.TemporaryDirectory() as tmpdir:
            ris_path = Path(tmpdir) / "import.ris"
            ris_path.write_text("TY  - JOUR\nTI  - tarunAI Connect Write E2E RIS\nER  - \n", encoding="utf-8")
            ris_result = self.run_cli(["--json", "import", "file", str(ris_path), "--collection", target, "--tag", "tarunai-connect-e2e"])
            ...
            json_path.write_text(
                json.dumps([{"itemType": "journalArticle", "title": "tarunAI Connect Write E2E JSON"}], ensure_ascii=False),
                encoding="utf-8",
            )
            json_result = self.run_cli(["--json", "import", "json", str(json_path), "--collection", target, "--tag", "tarunai-connect-e2e"])
    ...
    def test_opt_in_import_json_with_inline_attachment(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            title = f"tarunAI Connect Attachment E2E {uuid.uuid4().hex[:8]}"
            ...
            import_result = self.run_cli(
                ["--json", "import", "json", str(json_path), "--collection", target, "--tag", "tarunai-connect-e2e"]
            )
    ...
    def test_opt_in_note_add_command(self):
        assert SAMPLE_ITEM is not None
        result = self.run_cli(["--json", "note", "add", str(SAMPLE_ITEM["itemID"]), "--text", "tarunAI Connect write note"])
        self.assertEqual(result.returncode, 0, msg=result.stderr)
