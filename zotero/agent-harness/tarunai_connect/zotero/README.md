...
`tarunai-connect-zotero` is an agent-native CLI for Zotero desktop. It does not
...
If `tarunai-connect-zotero` is not recognized afterwards, your Python Scripts
directory is likely not on `PATH`. You can still use:

```bash
py -m tarunai_connect.zotero --help
```
...
tarunai-connect-zotero --json collection list
tarunai-connect-zotero --json item list --limit 10
...
```bash
tarunai-connect-zotero --json import file .\paper.ris --collection COLLAAAA --tag review
tarunai-connect-zotero --json import json .\items.json --collection COLLAAAA --tag imported
tarunai-connect-zotero --json import file .\paper.ris --collection COLLAAAA --attachments-manifest .\attachments.json
tarunai-connect-zotero --json import json .\items-with-pdf.json --collection COLLAAAA --attachment-timeout 90
```
...
```bash
tarunai-connect-zotero --json collection find "robotics"
```
...
```bash
tarunai-connect-zotero --json item find "foundation model"
tarunai-connect-zotero --json item find "A Very Specific Paper Title" --exact-title
tarunai-connect-zotero --json item find "vision" --collection COLLAAAA --limit 10
```
...
```bash
tarunai-connect-zotero --json collection items COLLAAAA
tarunai-connect-zotero --json item get REG12345
tarunai-connect-zotero --json item attachments REG12345
tarunai-connect-zotero --json item file REG12345
```
...
```bash
tarunai-connect-zotero --json item notes REG12345
tarunai-connect-zotero --json note get NOTEKEY
```
...
```bash
tarunai-connect-zotero --json note add REG12345 --text "Key takeaway: ..."
tarunai-connect-zotero --json note add REG12345 --file .\summary.md --format markdown
```
...
```bash
tarunai-connect-zotero --json item export REG12345 --format bibtex
tarunai-connect-zotero --json item export REG12345 --format csljson
tarunai-connect-zotero --json item citation REG12345 --style apa --locale en-US
tarunai-connect-zotero --json item bibliography REG12345 --style apa --locale en-US
```
...
```bash
tarunai-connect-zotero --json item context REG12345 --include-notes --include-links --include-bibtex
```
...
```bash
set OPENAI_API_KEY=...
tarunai-connect-zotero --json item analyze REG12345 --question "What is this paper's likely contribution?" --model gpt-5.4-mini --include-notes
```
...
```bash
tarunai-connect-zotero --json collection create "New Topic" --parent COLLAAAA --experimental
tarunai-connect-zotero --json item add-to-collection REG12345 COLLBBBB --experimental
tarunai-connect-zotero --json item move-to-collection REG67890 COLLAAAA --from COLLBBBB --experimental
tarunai-connect-zotero --json item move-to-collection REG67890 COLLAAAA --all-other-collections --experimental
```
...
set TARUNAI_CONNECT_FORCE_INSTALLED=1
py -m pytest tarunai_connect/zotero/tests/test_cli_entrypoint.py -v
py -m pytest tarunai_connect/zotero/tests/test_full_e2e.py -v -s
```
...
```bash
set TARUNAI_CONNECT_ZOTERO_ENABLE_WRITE_E2E=1
set TARUNAI_CONNECT_ZOTERO_IMPORT_TARGET=<collection-key-or-id>
py -m pytest tarunai_connect/zotero/tests/test_full_e2e.py -v -s
```
