...
from tarunai_connect.zotero.core.discovery import RuntimeContext
from tarunai_connect.zotero.utils import zotero_http, zotero_sqlite
...
def _read_json_items(path: Path) -> list[dict[str, Any]]:
    ...
    for index, item in enumerate(payload, start=1):
        ...
        copied = dict(item)
        copied.setdefault("id", f"tarunai-connect-zotero-{index}")
...
def enable_local_api(
    ...
    *,
    ...
    from tarunai_connect.zotero.utils import zotero_paths  # local import to avoid cycle
    ...
    if launch:
        from tarunai_connect.zotero.core import discovery  # local import to avoid cycle
