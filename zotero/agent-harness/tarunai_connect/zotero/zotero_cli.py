...
from tarunai_connect.zotero import __version__
from tarunai_connect.zotero.core import analysis, catalog, discovery, experimental, imports, notes, rendering, session as session_mod
from tarunai_connect.zotero.utils.repl_skin import ReplSkin
...
def repl_help_text() -> str:
    return """Interactive REPL for tarunai-connect-zotero
...
def _safe_print_banner(skin: ReplSkin) -> None:
    if not _supports_fancy_repl_output():
        click.echo("tarunai-connect-zotero REPL")
...
def dispatch(argv: list[str] | None = None, prog_name: str | None = None) -> int:
    ...
    try:
        result = cli.main(args=args, prog_name=prog_name or "tarunai-connect-zotero", standalone_mode=False)
