...
from tarunai_connect.inkscape.utils.svg_utils import (
    SVG_NS, INKSCAPE_NS, SODIPODI_NS, reset_id_counter,
    parse_style, serialize_svg,
)
from tarunai_connect.inkscape.core.document import (
    create_document, save_document, open_document, save_svg,
    get_document_info, project_to_svg,
)
from tarunai_connect.inkscape.core.shapes import (
    add_rect, add_circle, add_ellipse, add_line, add_polygon,
    add_path, add_star, remove_object, duplicate_object, list_objects,
)
from tarunai_connect.inkscape.core.text import add_text, set_text_property, list_text_objects
from tarunai_connect.inkscape.core.styles import set_fill, set_stroke, set_opacity, set_style, get_object_style
from tarunai_connect.inkscape.core.transforms import translate, rotate, scale, get_transform, clear_transform
from tarunai_connect.inkscape.core.layers import add_layer, remove_layer, move_to_layer, list_layers, get_layer
from tarunai_connect.inkscape.core.paths import (
    path_union, path_intersection, path_difference,
    convert_to_path,
)
from tarunai_connect.inkscape.core.gradients import add_linear_gradient, add_radial_gradient, apply_gradient
from tarunai_connect.inkscape.core.export import render_to_png, export_svg, list_presets
from tarunai_connect.inkscape.core.session import Session
...
class TestCLISubprocess:
    CLI_BASE = _resolve_cli("tarunai-connect-inkscape")
...
class TestInkscapeBackend:
    ...
    def test_inkscape_is_installed(self):
        from tarunai_connect.inkscape.utils.inkscape_backend import find_inkscape
        path = find_inkscape()
        assert os.path.exists(path)
        print(f"\n  Inkscape binary: {path}")

    def test_inkscape_version(self):
        from tarunai_connect.inkscape.utils.inkscape_backend import get_version
        version = get_version()
...
class TestInkscapeExportE2E:
    ...
    def test_svg_to_png(self):
        ...
        from tarunai_connect.inkscape.utils.inkscape_backend import export_svg_to_png
...
<svg xmlns="http://www.w3.org/2000/svg" width="200" height="150" viewBox="0 0 200 150">
  ...
  <text x="100" y="80" text-anchor="middle" fill="white" font-size="20">Test</text>
    ...
    def test_svg_to_pdf(self):
        ...
        from tarunai_connect.inkscape.utils.inkscape_backend import export_svg_to_pdf
