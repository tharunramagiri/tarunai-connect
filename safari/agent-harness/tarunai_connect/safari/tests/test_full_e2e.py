"""E2E tests for tarunai-connect-safari — Requires Safari + macOS.
...
To also enforce the installed command (not module fallback):
    TARUNAI_CONNECT_FORCE_INSTALLED=1 SAFARI_E2E=1 \\
        python -m pytest tarunai_connect/safari/tests/test_full_e2e.py -v -s
"""
...
from tarunai_connect.safari.utils.safari_backend import is_available
from tarunai_connect.safari.safari_cli import cli
...
def _resolve_cli(name: str):
    ...
    module = "tarunai_connect.safari.safari_cli"
...
class TestCLISubprocess:
    ...
    actual installed `tarunai-connect-safari` command via subprocess, not just
    ...
    ``_resolve_cli`` (which can raise when ``TARUNAI_CONNECT_FORCE_INSTALLED=1``
    ...
    def CLI_BASE(self) -> list[str]:
        ...
        if base is None:
            base = _resolve_cli("tarunai-connect-safari")
