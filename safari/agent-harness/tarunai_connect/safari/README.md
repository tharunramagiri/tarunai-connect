...
tarunai-connect-safari tools describe safari_click
...
Run `tarunai-connect-safari <command> --help` for details.
...
| `TARUNAI_CONNECT_SAFARI_ALLOWED_SCHEMES` | Override allowed URL schemes        |
| `TARUNAI_CONNECT_FORCE_INSTALLED`        | Test mode: require installed CLI    |
...
```bash
tarunai-connect-safari --json tool snapshot > /tmp/snap.json
# Agent reads /tmp/snap.json, finds "Submit" button with ref "3_12"
tarunai-connect-safari tool click --ref 3_12
```
...
```bash
tarunai-connect-safari tool new-tab --url https://example.com
tarunai-connect-safari tool click --selector "#button"
```
...
This means **running `tarunai-connect-safari` (or any other safari-mcp
...
Then run `python -m pytest tarunai_connect/safari/tests/test_parity.py` —
...
- [Harness architecture deep-dive](https://github.com/tharunramagiri/tarunai-connect/blob/main/safari/agent-harness/HARNESS.md)
- [Safari-specific analysis](https://github.com/tharunramagiri/tarunai-connect/blob/main/safari/agent-harness/SAFARI.md)
- [Test plan & results](https://github.com/tharunramagiri/tarunai-connect/blob/main/safari/agent-harness/tarunai_connect/safari/tests/TEST.md)
