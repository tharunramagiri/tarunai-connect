...
_SESSION_DIR = Path.home() / ".tarunai-connect-calibre"
...
def require_library(session: dict | None = None) -> str:
    ...
    if not path:
        raise RuntimeError(
            ...
            "Set one with: tarunai-connect-calibre library connect <path>\n"
            "Or set CALIBRE_LIBRARY environment variable."
        )
    if not Path(path).exists():
        raise RuntimeError(
            f"Library path does not exist: {path}\n"
            "Set a valid path with: tarunai-connect-calibre library connect <path>"
        )
    return path
