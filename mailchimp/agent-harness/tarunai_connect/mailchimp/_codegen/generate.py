...
Usage:
    python -m tarunai_connect.mailchimp._codegen.generate

Downloads the Mailchimp Marketing API spec and emits one Python module per
tag into tarunai_connect/mailchimp/commands/. The generated files are committed
...
def _generate_command_func(
    ...
    lines.append("    from tarunai_connect.mailchimp.core.client import get_client, MailchimpError")
    lines.append("    from tarunai_connect.mailchimp.utils.output import _out, _out_ok, _out_err")
...
def _generate_module(tag: str, operations: list[tuple[str, str, dict]]) -> str:
    ...
    header_lines = [
        ...
        "",
        "Auto-generated by tarunai_connect/mailchimp/_codegen/generate.py.",
...
def generate(spec: dict, out_dir: Path = OUT_DIR) -> None:
    ...
    init_lines = ['"""Auto-generated command groups for tarunai-connect-mailchimp."""', ""]
    for mod in sorted(generated):
        group_var = f"{mod}_group"
        init_lines.append(f"from tarunai_connect.mailchimp.commands.{mod} import {group_var}")
    init_lines.append("")
