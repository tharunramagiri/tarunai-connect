"""Tests for tarunai-connect — registry, installer, analytics, and CLI."""
...
from tarunai_connect import __version__
from tarunai_connect.registry import fetch_registry, fetch_all_clis, get_cli, search_clis, list_categories
from tarunai_connect.preview import (
    inspect_bundle,
    inspect_session,
    open_in_browser,
    render_html,
...
from tarunai_connect.installer import (
    install_cli,
    uninstall_cli,
    get_installed,
    _load_installed,
...
from tarunai_connect.analytics import _is_enabled, track_event, track_install, track_uninstall as analytics_track_uninstall, track_visit, track_first_run, _detect_is_agent, detect_invocation_context
from tarunai_connect.cli import main


# ─── Sample registry data ─────────────────────────────────────────────

SAMPLE_REGISTRY = {
    "meta": {"repo": "https://github.com/tharunramagiri/TarunAI Connect", "description": "test"},
    "clis": [
        {
            "name": "gimp",
            "display_name": "GIMP",
...
class TestRegistry:
    ...
    @patch("tarunai_connect.registry.requests.get")
    @patch("tarunai_connect.registry.CACHE_FILE", Path(tempfile.mktemp()))
    ...
    @patch("tarunai_connect.registry.requests.get", side_effect=requests.ConnectionError("network down"))
    def test_fetch_registry_uses_cache_on_refresh_failure(self, mock_get, tmp_path):
        ...
        with patch("tarunai_connect.registry.CACHE_FILE", cache_file):
            result = fetch_registry(force_refresh=True)

        assert result["clis"][0]["name"] == "gimp"
        mock_get.assert_called_once()

    @patch("tarunai_connect.registry.fetch_public_registry", return_value=None)
    @patch("tarunai_connect.registry.fetch_registry")
    ...
    @patch("tarunai_connect.registry.fetch_all_clis", return_value=SAMPLE_REGISTRY["clis"])
...
class TestPreviewBundle:
    ...
    def test_render_html(self, tmp_path):
        ...
        assert "TarunAI Connect Preview Bundle" in content
    ...
    def test_render_live_html(self, tmp_path):
        ...
        assert rendered == str(output_path.resolve())
        content = output_path.read_text()
        assert "TarunAI Connect Live Preview Session" in content
    ...
    @patch("tarunai_connect.preview.subprocess.Popen")
    @patch("tarunai_connect.preview.shutil.which")
...
class TestInstaller:
    ...
    def test_load_installed_empty(self, tmp_path):
        with patch("tarunai_connect.installer.INSTALLED_FILE", tmp_path / "installed.json"):
            assert _load_installed() == {}

    def test_save_and_load_installed(self, tmp_path):
        installed_file = tmp_path / "installed.json"
        with patch("tarunai_connect.installer.INSTALLED_FILE", installed_file):
            _save_installed({"gimp": {"version": "1.0.0"}})
            data = _load_installed()
            assert data["gimp"]["version"] == "1.0.0"

    @patch("tarunai_connect.installer.subprocess.run")
    @patch("tarunai_connect.installer.get_cli")
    @patch("tarunai_connect.installer.INSTALLED_FILE", Path(tempfile.mktemp()))
    ...
    @patch("tarunai_connect.installer.subprocess.run", side_effect=FileNotFoundError(2, "No such file or directory", "brew"))
    @patch("tarunai_connect.installer.get_cli")
    @patch("tarunai_connect.installer.INSTALLED_FILE", Path(tempfile.mktemp()))
    ...
    @patch("tarunai_connect.installer.shutil.which", return_value="/usr/local/bin/obsidian")
    @patch("tarunai_connect.installer.get_cli")
    @patch("tarunai_connect.installer.INSTALLED_FILE", Path(tempfile.mktemp()))
...
class TestUvStrategy:
    ...
    @patch("tarunai_connect.installer.get_cli")
    @patch("tarunai_connect.installer.INSTALLED_FILE", Path(tempfile.mktemp()))
    @patch("tarunai_connect.installer._find_uv", return_value="/usr/bin/uv")
    ...
    @patch("tarunai_connect.installer.get_cli")
    @patch("tarunai_connect.installer._find_uv", return_value=None)
    ...
    def test_update_uv_success(self, mock_find_uv, mock_get_cli, mock_run):
        mock_get_cli.return_value = GENERATE_VEO_CLI
        mock_run.return_value = MagicMock(returncode=0, stdout="", stderr="")
        from tarunai_connect.installer import update_cli
...
class TestAnalytics:
    ...
    def test_analytics_disabled_by_env(self):
        with patch.dict(os.environ, {"TARUNAI_CONNECT_NO_ANALYTICS": "1"}):
            assert not _is_enabled()

    def test_analytics_disabled_by_true(self):
        with patch.dict(os.environ, {"TARUNAI_CONNECT_NO_ANALYTICS": "true"}):
            assert not _is_enabled()

    @patch("tarunai_connect.analytics._send_event")
    def test_track_event_sends_request(self, mock_send):
        with patch.dict(os.environ, {}, clear=True):
            ...
            assert payload["properties"]["hostname"] == "tarunai.dev"
    ...
    def test_track_event_supports_umami_provider(self, mock_send):
        with patch.dict(os.environ, {"TARUNAI_CONNECT_ANALYTICS_PROVIDER": "umami"}, clear=False):
            track_event("test-event")
            import time
            time.sleep(0.2)
            mock_send.assert_called_once()
            ...
            assert payload["payload"]["hostname"] == "tarunai.dev"
    ...
    def test_track_install_event_name_is_flat(self, mock_send):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            assert payload["properties"]["$current_url"] == "https://tarunai.dev/tarunai-connect/install/gimp"
    ...
    def test_track_uninstall_event_name_is_flat(self, mock_send):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            assert payload["properties"]["$current_url"] == "https://tarunai.dev/tarunai-connect/uninstall/blender"
    ...
    def test_track_launch_fires(self, mock_send):
        ...
        from tarunai_connect.analytics import track_launch
        with patch.dict(os.environ, {}, clear=True):
            track_launch("gimp")
            import time
            time.sleep(0.2)
            ...
            assert payload["properties"]["cli"] == "gimp"
            assert payload["properties"]["$current_url"] == "https://tarunai.dev/tarunai-connect/launch/gimp"

    @patch("tarunai_connect.analytics._send_event")
    def test_track_visit_human(self, mock_send):
        """tarunai-connect call event sent when not detected as agent."""
        with patch.dict(os.environ, {}, clear=True):
            ...
            mock_send.assert_called_once()
            payload = mock_send.call_args[0][0]
            assert payload["event"] == "tarunai-connect call"
            assert payload["properties"]["$current_url"] == "https://tarunai.dev/tarunai-connect/call"
    ...
    def test_track_visit_agent(self, mock_send):
        """tarunai-connect call event captures the agent flag."""
        with patch.dict(os.environ, {}, clear=True):
    ...
    @patch("tarunai_connect.analytics._parent_process_commands", return_value=["/usr/local/bin/codex --run"])
    ...
    @patch("tarunai_connect.analytics._parent_process_commands")
    ...
    @patch("tarunai_connect.analytics._parent_process_commands", return_value=[])
    ...
    def test_first_run_sends_event(self, mock_send, tmp_path):
        """First invocation sends tarunai-connect-installed event."""
        with patch.dict(os.environ, {"HOME": str(tmp_path)}, clear=False):
            ...
            mock_send.assert_called_once()
            payload = mock_send.call_args[0][0]
            assert payload["event"] == "tarunai-connect-installed"
            assert payload["properties"]["$current_url"] == "https://tarunai.dev/tarunai-connect/installed"
            # Marker file should now exist
            assert (tmp_path / ".tarunai-connect" / ".first_run_sent").exists()

    @patch("tarunai_connect.analytics._send_event")
    def test_first_run_skips_if_marker_exists(self, mock_send, tmp_path):
        """Second invocation does NOT send tarunai-connect-installed event."""
        cli_hub_dir = tmp_path / ".tarunai-connect"
        cli_hub_dir.mkdir()
...
class TestCLI:
    ...
    @patch("tarunai_connect.cli.track_first_run")
    ...
    def test_help(self, mock_detect, mock_visit, mock_first_run):
        ...
        assert "tarunai-connect" in result.output
        assert result.exit_code == 0

    @patch("tarunai_connect.cli.track_first_run")
    @patch("tarunai_connect.cli.track_visit")
    @patch("tarunai_connect.cli.detect_invocation_context")
    @patch("tarunai_connect.cli.fetch_all_clis", return_value=SAMPLE_REGISTRY["clis"])
    @patch("tarunai_connect.cli.list_categories", return_value=["3d", "audio", "image"])
    @patch("tarunai_connect.cli.get_installed", return_value={})
    ...
    @patch("tarunai_connect.cli.track_visit")
    @patch("tarunai_connect.cli.detect_invocation_context")
    @patch("tarunai_connect.cli.search_clis", return_value=[SAMPLE_REGISTRY["clis"][0]])
    @patch("tarunai_connect.cli.get_installed", return_value={})
    ...
    def test_info_command(self, mock_installed, mock_get, mock_detect, mock_visit, mock_first_run):
        ...
        assert "Install: tarunai-connect install gimp" in result.output
        assert result.exit_code == 0

    @patch("tarunai_connect.cli.track_first_run")
    @patch("tarunai_connect.cli.track_visit")
    @patch("tarunai_connect.cli.detect_invocation_context")
    @patch("tarunai_connect.cli.get_cli", return_value=None)
    ...
    @patch("tarunai_connect.cli.track_visit")
    @patch("tarunai_connect.cli.detect_invocation_context")
    @patch("tarunai_connect.cli.track_uninstall")
    @patch("tarunai_connect.cli.uninstall_cli", return_value=(True, "Uninstalled GIMP"))
    ...
    def test_visit_agent_on_invocation(self, mock_detect, mock_visit, mock_first_run):
        """When agent env detected, track_visit is called with the new tarunai-connect call metadata."""
    ...
    @patch("tarunai_connect.cli.track_visit")
    @patch("tarunai_connect.cli.detect_invocation_context")
    @patch("tarunai_connect.cli.install_cli", return_value=(True, "Installed Jimeng / Dreamina CLI (dreamina)"))
    @patch("tarunai_connect.cli.get_cli", return_value={**SAMPLE_REGISTRY["clis"][0], "entry_point": "dreamina", "name": "jimeng", "display_name": "Jimeng / Dreamina CLI", "version": "latest", "_source": "public"})
    @patch("tarunai_connect.cli.track_install")
    def test_install_shows_launch_hint(self, mock_track, mock_get, mock_install, mock_detect, mock_visit, mock_first_run):
        """Post-install output includes both entry point and tarunai-connect launch hint."""
        mock_detect.return_value = self.human_detection
        result = self.runner.invoke(main, ["install", "jimeng"])
        assert result.exit_code == 0
        assert "dreamina" in result.output
        assert "tarunai-connect launch jimeng" in result.output

    @patch("tarunai_connect.cli.track_first_run")
    @patch("tarunai_connect.cli.track_visit")
    @patch("tarunai_connect.cli.detect_invocation_context")
    @patch("tarunai_connect.cli.shutil.which", return_value="/usr/bin/dreamina")
    @patch("tarunai_connect.cli.os.execvp")
    @patch("tarunai_connect.cli.get_cli", return_value=JIMENG_CLI)
    ...
    @patch("tarunai_connect.cli.track_visit")
    @patch("tarunai_connect.cli.detect_invocation_context")
    @patch("tarunai_connect.cli.shutil.which", return_value=None)
    @patch("tarunai_connect.cli.get_cli", return_value=JIMENG_CLI)
    def test_launch_not_on_path_shows_install_hint(self, mock_get, mock_which, mock_detect, mock_visit, mock_first_run):
        ...
        assert result.exit_code == 1
        assert "tarunai-connect install jimeng" in result.output
