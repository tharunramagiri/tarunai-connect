"""tarunai-connect — CLI entry point."""
...
from tarunai_connect.registry import fetch_all_clis, get_cli, search_clis, list_categories
from tarunai_connect.installer import install_cli, uninstall_cli, get_installed, update_cli
from tarunai_connect.analytics import (
...
def main(ctx, version):
    """tarunai-connect — Download and manage TarunAI Connect harnesses and public CLIs."""
    track_first_run()
    track_visit(command=_invocation_command(ctx, version), detection=detect_invocation_context())
    if version:
        click.echo(f"tarunai-connect {__version__}")
        return
...
def install(name):
    ...
    if success:
        ...
        if cli:
            ...
            click.echo(f"  Or launch:   tarunai-connect launch {cli['name']}")
...
def search(query, as_json):
    ...
    for cli in results:
        ...
        click.echo(f"    Install: tarunai-connect install {cli['name']}")
...
def info(name):
    ...
    click.echo(f"\n  Install: tarunai-connect install {cli['name']}")
    click.echo()
...
def launch(name, args):
    ...
    if not shutil.which(entry):
        click.secho(
            f"'{entry}' not found on PATH. Install it first: tarunai-connect install {name}",
            fg="red",
            err=True,
        )
        raise SystemExit(1)
    ...
    if auto_open:
        ...
        if launched.get("launched"):
            ...
            click.echo(
                ...
                f"  {live_url}\n"
                f"  Suggested command: {session.get('watch_command') or f'tarunai-connect previews watch {session_dir} --open'}"
            )
