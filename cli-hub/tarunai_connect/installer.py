...
from tarunai_connect.registry import get_cli

INSTALLED_FILE = Path.home() / ".tarunai-connect" / "installed.json"
...
def install_cli(name):
    ...
    cli = get_cli(name)
    if cli is None:
        return False, f"CLI '{name}' not found in registry. Use 'tarunai-connect list' to see available CLIs."
