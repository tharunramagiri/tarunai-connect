...
HOSTNAME = "tarunai.dev"
USER_AGENT = f"Mozilla/5.0 (compatible; tarunai-connect/{__version__})"
...
def _is_enabled():
    return os.environ.get("TARUNAI_CONNECT_NO_ANALYTICS", "").strip() not in ("1", "true", "yes")


def _provider():
    provider = os.environ.get("TARUNAI_CONNECT_ANALYTICS_PROVIDER", ANALYTICS_PROVIDER).strip().lower()
    return provider if provider in {"posthog", "umami"} else ANALYTICS_PROVIDER


def _analytics_dir():
    return Path.home() / ".tarunai-connect"
...
def detect_invocation_context():
    """Classify the current tarunai-connect invocation as human, agent, or scripted."""
    signals = []
...
def _get_distinct_id():
    override = os.environ.get("TARUNAI_CONNECT_ANALYTICS_DISTINCT_ID", "").strip()
    if override:
        return override
    ...
    try:
        ...
        return f"tarunai-connect-anon-{uuid.uuid4()}"


def _posthog_capture_url():
    host = os.environ.get("TARUNAI_CONNECT_POSTHOG_API_HOST", POSTHOG_API_HOST).rstrip("/")
...
def _build_posthog_payload(event_name, url, data):
    return {
        "api_key": os.environ.get("TARUNAI_CONNECT_POSTHOG_PROJECT_TOKEN", POSTHOG_PROJECT_TOKEN),
        "event": event_name,
...
def track_event(event_name, url="/tarunai-connect", data=None):
...
def track_install(cli_name, version):
    ...
    track_event("cli-install", url=f"/tarunai-connect/install/{cli_name}", data={
        "cli": cli_name,
        "version": version,
...
def track_uninstall(cli_name):
    ...
    track_event("cli-uninstall", url=f"/tarunai-connect/uninstall/{cli_name}", data={
        "cli": cli_name,
        "platform": platform.system().lower(),
    })
...
def track_launch(cli_name):
    """Track a CLI launch event — fires when a user runs `tarunai-connect launch <name>`.
    Distinct from install: this is actual usage signal."""
    track_event("cli-launch", url=f"/tarunai-connect/launch/{cli_name}", data={
        "cli": cli_name,
        "platform": platform.system().lower(),
    })
...
def track_visit(is_agent=False, command="root", detection=None):
    """Track a tarunai-connect invocation using the new tarunai-connect call event."""
    stdin_tty = _stdin_is_tty()
    ...
    track_event("tarunai-connect call", url="/tarunai-connect/call", data={
        "command": command,
...
def track_first_run():
    """Send a one-time 'tarunai-connect-installed' event on first invocation."""
    marker = _analytics_dir() / ".first_run_sent"
    if marker.exists():
        return
    track_event("tarunai-connect-installed", url="/tarunai-connect/installed", data={
        "version": __version__,
        "platform": platform.system().lower(),
    })
    try:
...
def _detect_is_agent():
    """Detect if tarunai-connect is likely being invoked by an AI agent."""
