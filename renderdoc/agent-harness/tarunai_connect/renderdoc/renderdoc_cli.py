...
def _get_handle(ctx: click.Context):
    ...
    if _capture_handle is not None:
        ...
        _capture_handle = None
        _capture_handle_path = None
    from tarunai_connect.renderdoc.core.capture import CaptureHandle
    from tarunai_connect.renderdoc.utils.errors import handle_error
    ...
    try:
        ...
        if ctx.obj.get("json_mode"):
            from tarunai_connect.renderdoc.utils.output import output_json
            output_json(err)
            ctx.exit(1)
        else:
...
@click.version_option(package_name="tarunai-connect-renderdoc")
@click.pass_context
...
def actions_list(ctx, flat, draws_only):
    ...
    handle = _get_handle(ctx)
    from tarunai_connect.renderdoc.core.actions import list_actions, get_drawcalls_only
...
def actions_summary(ctx):
    ...
    handle = _get_handle(ctx)
    from tarunai_connect.renderdoc.core.actions import action_summary
...
def actions_find(ctx, pattern):
    ...
    handle = _get_handle(ctx)
    from tarunai_connect.renderdoc.core.actions import find_actions_by_name
...
def actions_get(ctx, event_id):
    ...
    handle = _get_handle(ctx)
    from tarunai_connect.renderdoc.core.actions import find_action_by_event
...
def textures_list(ctx):
    ...
    handle = _get_handle(ctx)
    from tarunai_connect.renderdoc.core.textures import list_textures
...
def textures_get(ctx, resource_id):
    ...
    handle = _get_handle(ctx)
    from tarunai_connect.renderdoc.core.textures import get_texture
...
def textures_save(ctx, resource_id, output, fmt, mip, slice_idx, alpha):
    ...
    handle = _get_handle(ctx)
    from tarunai_connect.renderdoc.core.textures import save_texture
...
def textures_save_outputs(ctx, event_id, output_dir, fmt):
    ...
    handle = _get_handle(ctx)
    from tarunai_connect.renderdoc.core.textures import save_action_outputs
...
def textures_pick(ctx, resource_id, x, y, mip, slice_idx):
    ...
    handle = _get_handle(ctx)
    from tarunai_connect.renderdoc.core.textures import pick_pixel
...
def pipeline_state(ctx, event_id):
    ...
    handle = _get_handle(ctx)
    from tarunai_connect.renderdoc.core.pipeline import get_pipeline_state
...
def pipeline_shader_export(ctx, event_id, stage, output_dir):
    ...
    """
    handle = _get_handle(ctx)
    from tarunai_connect.renderdoc.core.pipeline import export_shader
...
def pipeline_dump_shader_reflection(ctx, event_id, stage, output_dir):
    ...
    """
    handle = _get_handle(ctx)
    from tarunai_connect.renderdoc.core.pipeline import export_shader_reflection
    ...
    if output_dir is None:
        ...
        from tarunai_connect.renderdoc.core.pipeline import STAGE_MAP
...
def pipeline_dump(ctx, event_id, output_path):
    ...
    """
    handle = _get_handle(ctx)
    from tarunai_connect.renderdoc.core.pipeline import dump_pipeline
...
def pipeline_cbuffer(ctx, event_id, stage, cbuffer_index):
    ...
    handle = _get_handle(ctx)
    from tarunai_connect.renderdoc.core.pipeline import get_cbuffer_contents
...
def resources_list(ctx):
    ...
    handle = _get_handle(ctx)
    from tarunai_connect.renderdoc.core.resources import list_resources
...
def resources_buffers(ctx):
    ...
    handle = _get_handle(ctx)
    from tarunai_connect.renderdoc.core.resources import list_buffers
...
def resources_read_buffer(ctx, resource_id, offset, length, fmt):
    ...
    handle = _get_handle(ctx)
    from tarunai_connect.renderdoc.core.resources import get_buffer_data
...
def mesh_inputs(ctx, event_id, max_vertices):
    ...
    handle = _get_handle(ctx)
    from tarunai_connect.renderdoc.core.mesh import get_mesh_inputs
...
def mesh_outputs(ctx, event_id, max_vertices):
    ...
    handle = _get_handle(ctx)
    from tarunai_connect.renderdoc.core.mesh import get_mesh_outputs
...
def counters_list(ctx):
    ...
    handle = _get_handle(ctx)
    from tarunai_connect.renderdoc.core.counters import list_counters
...
def counters_fetch(ctx, ids):
    ...
    handle = _get_handle(ctx)
    from tarunai_connect.renderdoc.core.counters import fetch_counters
...
def pipeline_diff_cmd(ctx, event_a, event_b, capture_b, compact, output):
    ...
    Examples:
      ...
      tarunai-connect-renderdoc -c frame.rdc pipeline diff 100 200
      # Custom output path
    ...
    from tarunai_connect.renderdoc.core.diff import diff_pipeline
...
def preview_recipes(ctx):
    ...
    from tarunai_connect.renderdoc.core.preview import list_recipes
...
def preview_capture_cmd(ctx, recipe, event_id, force, root_dir):
    ...
    from tarunai_connect.renderdoc.core.preview import capture
    ...
    result = capture(
        ...
        force=force,
        root_dir=root_dir,
        command=f"tarunai-connect-renderdoc -c {capture_path} preview capture --recipe {recipe}",
    )
    _output(ctx, result)
...
def preview_diff_cmd(ctx, event_a, event_b, capture_b, compact, force, root_dir):
    ...
    from tarunai_connect.renderdoc.core.preview import diff
    ...
    result = diff(
        ...
        root_dir=root_dir,
        command=(
            f"tarunai-connect-renderdoc -c {capture_path_a} preview diff {event_a} {event_b}"
...
def preview_latest_cmd(ctx, recipe, bundle_kind, root_dir):
    ...
    from tarunai_connect.renderdoc.core.preview import latest
...
def repl(ctx):
    ...
    from tarunai_connect.renderdoc.utils.repl_skin import ReplSkin
