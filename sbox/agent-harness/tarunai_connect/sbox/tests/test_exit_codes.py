...
Reviewer feedback at tharunramagiri/tarunai-connect PR #251 flagged that handlers were
...
The subprocess tests use ``python -m tarunai_connect.sbox`` rather than the
installed ``tarunai-connect-sbox`` binary so the suite is portable across
...
CLI = [sys.executable, "-m", "tarunai_connect.sbox"]
...
class TestAuditedFailurePaths:
    ...
    def test_asset_compile_returns_success_false_exits_one(self, monkeypatch):
        ...
        from tarunai_connect.sbox import sbox_cli
