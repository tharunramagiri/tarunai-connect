# Test Plan - tarunai-connect-sbox
...
python -m pytest tarunai_connect/sbox/tests/test_core.py -v

# Orchestrator tests (Pillow required - already a hard dependency)
python -m pytest tarunai_connect/sbox/tests/test_orchestrator.py -v

# Full e2e suite (TestE2EBackend skips automatically when s&box is not installed)
python -m pytest tarunai_connect/sbox/tests/test_full_e2e.py -v

# Everything
python -m pytest tarunai_connect/sbox/tests/ -v
```
...
| `TARUNAI_CONNECT_FORCE_INSTALLED` | When set to `1`/`true`/`yes`, `TestCLISubprocess` runs against the installed `tarunai-connect-sbox` console script instead of `python -m`. |
...
|---|---|---|
| `TestCLISubprocess` | 40 | Spawns `tarunai-connect-sbox` as a subprocess (or `python -m tarunai_connect.sbox.sbox_cli` when not installed) and asserts `--json` output for every documented command group |
