...
def test_import():
    import tarunai_connect.rekordbox  # noqa


def test_cli_help():
    """`--help` should exit 0."""
    r = subprocess.run([sys.executable, "-m", "tarunai_connect.rekordbox", "--help"],
...
def test_subcommand_help():
    for sub in ["library", "playlist", "deck", "status", "install-mapping", "mix"]:
        r = subprocess.run([sys.executable, "-m", "tarunai_connect.rekordbox", sub, "--help"],
                           capture_output=True, text=True)
        assert r.returncode == 0, f"{sub} --help failed: {r.stderr}"


def test_playlist_write_options_are_documented():
    r = subprocess.run([sys.executable, "-m", "tarunai_connect.rekordbox", "playlist", "create", "--help"],
                       capture_output=True, text=True)
...
def test_write_guard_refuses_running_rekordbox_without_force(monkeypatch):
    import tarunai_connect.rekordbox.rekordbox_cli as rb
...
def test_write_guard_creates_backup_before_write(monkeypatch, tmp_path):
    ...
    assert backup_path.startswith(str(tmp_path / "tarunai-connect-backups"))
...
def test_data_file_present():
    ...
    import tarunai_connect.rekordbox as pkg
