"""Unit tests for tarunai-connect-cloudcompare core modules.
...
class TestCreateProject:
    def test_creates_file(self, project_path):
        from tarunai_connect.cloudcompare.core.project import create_project
...
class TestLoadProject:
    def test_loads_existing_project(self, project_path):
        from tarunai_connect.cloudcompare.core.project import create_project, load_project
        create_project(project_path)
        proj = load_project(project_path)
        assert proj["version"] == "1.0"

    def test_raises_on_missing_file(self, tmp_dir):
        from tarunai_connect.cloudcompare.core.project import load_project
...
class TestSaveProject:
    def test_saves_and_updates_modified_at(self, project_path):
        from tarunai_connect.cloudcompare.core.project import create_project, save_project, load_project
        proj = create_project(project_path)
...
class TestAddCloud:
    def test_adds_cloud_entry(self, project_path, dummy_cloud_file):
        from tarunai_connect.cloudcompare.core.project import create_project, add_cloud
        proj = create_project(project_path)
...
class TestAddMesh:
    def test_adds_mesh_entry(self, project_path, dummy_mesh_file):
        from tarunai_connect.cloudcompare.core.project import create_project, add_mesh
        proj = create_project(project_path)
...
class TestRemoveCloud:
    def test_removes_cloud_by_index(self, project_path, dummy_cloud_file):
        from tarunai_connect.cloudcompare.core.project import create_project, add_cloud, remove_cloud
        proj = create_project(project_path)
    ...
    def test_raises_on_out_of_range_index(self, project_path):
        from tarunai_connect.cloudcompare.core.project import create_project, remove_cloud
        proj = create_project(project_path)
...
class TestGetCloud:
    def test_returns_cloud_by_index(self, project_path, dummy_cloud_file):
        from tarunai_connect.cloudcompare.core.project import create_project, add_cloud, get_cloud
        proj = create_project(project_path)
    ...
    def test_raises_on_invalid_index(self, project_path):
        from tarunai_connect.cloudcompare.core.project import create_project, get_cloud
        proj = create_project(project_path)
        with pytest.raises(IndexError):
            get_cloud(proj, 99)
...
class TestProjectInfo:
    def test_returns_summary(self, project_path, dummy_cloud_file, dummy_mesh_file):
        from tarunai_connect.cloudcompare.core.project import (
...
class TestRecordOperation:
    def test_appends_to_history(self, project_path):
        from tarunai_connect.cloudcompare.core.project import create_project, record_operation
        proj = create_project(project_path)
...
class TestSession:
    def test_creates_new_project_when_missing(self, tmp_dir):
        from tarunai_connect.cloudcompare.core.session import Session
...
class TestCCBackendConstants:
    def test_cloud_formats_has_las(self):
        from tarunai_connect.cloudcompare.utils.cc_backend import CLOUD_FORMATS
    ...
    def test_mesh_formats_has_obj(self):
        from tarunai_connect.cloudcompare.utils.cc_backend import MESH_FORMATS
    ...
    def test_find_cloudcompare_raises_when_not_found(self, monkeypatch):
        ...
        import shutil
        import subprocess
        from tarunai_connect.cloudcompare.utils import cc_backend
...
class TestCoordToSFValidation:
    def test_invalid_dimension_raises(self):
        from tarunai_connect.cloudcompare.utils.cc_backend import coord_to_sf
    ...
    def test_invalid_dimension_in_filter_raises(self):
        from tarunai_connect.cloudcompare.utils.cc_backend import coord_to_sf_and_filter
        with pytest.raises(ValueError, match="dimension must be"):
...
class TestNoiseFilterImport:
    def test_noise_filter_importable(self):
        ...
        from tarunai_connect.cloudcompare.utils.cc_backend import noise_filter
    ...
    def test_color_filter_removed(self):
        ...
        import tarunai_connect.cloudcompare.utils.cc_backend as backend
        assert not hasattr(backend, "color_filter")


class TestCSFFilterValidation:
    def test_invalid_scene_raises(self):
        from tarunai_connect.cloudcompare.utils.cc_backend import csf_filter
    ...
    def test_valid_scenes_accepted(self):
        ...
        from tarunai_connect.cloudcompare.utils.cc_backend import csf_filter, find_cloudcompare
