"""End-to-end tests for the PM2 tarunAI Connect harness.

These tests call the REAL pm2 binary and the installed tarunai-connect-pm2
CLI. They require:
  - pm2 installed globally (npm install -g pm2)
  - tarunai-connect-pm2 installed (pip install -e .)
...
def _resolve_cli() -> str:
    """Resolve the tarunai-connect-pm2 binary path.

    Checks: shutil.which, then common venv/bin locations.
    """
    binary = shutil.which("tarunai-connect-pm2")
    if binary:
        return binary

    # Try the venv bin dir that matches the running Python
    venv_bin = os.path.join(os.path.dirname(sys.executable), "tarunai-connect-pm2")
    if os.path.isfile(venv_bin):
        return venv_bin

    pytest.skip("tarunai-connect-pm2 binary not found on PATH")
...
def _run_cli(*args: str, timeout: int = 30) -> subprocess.CompletedProcess:
    """Run tarunai-connect-pm2 with the given arguments."""
    cli = _resolve_cli()
    cmd = [cli, *args]
...
class TestProcessListE2E:
    ...
    def test_process_list_json_returns_valid_json(self):
        """tarunai-connect-pm2 --json process list outputs parseable JSON."""
    ...
    def test_process_list_human_returns_zero(self):
        """tarunai-connect-pm2 process list exits 0."""
...
class TestSystemE2E:
    ...
    def test_system_version_json(self):
        """tarunai-connect-pm2 --json system version returns valid JSON with version key."""
    ...
    def test_system_version_human(self):
        """tarunai-connect-pm2 system version returns a version string."""
...
class TestHelpE2E:
    ...
    def test_help_flag_exits_zero(self):
        """tarunai-connect-pm2 --help exits with code 0."""
        result = _run_cli("--help")
        assert result.returncode == 0
        assert "tarunAI Connect PM2" in result.stdout or "Usage" in result.stdout

    def test_process_help(self):
        """tarunai-connect-pm2 process --help shows subcommands."""
...
class TestProcessMetricsE2E:
    ...
    def test_metrics_json_returns_list(self):
        """tarunai-connect-pm2 --json process metrics returns a JSON list."""
