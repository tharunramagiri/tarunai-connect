"""Unit tests for the PM2 tarunAI Connect harness.
...
class TestFindPm2:
    ...
    def test_find_pm2_found(self, mock_which):
        from tarunai_connect.pm2.utils.pm2_backend import _find_pm2
...
class TestRunPm2:
    ...
    @patch("tarunai_connect.pm2.utils.pm2_backend._get_pm2", return_value="/usr/bin/pm2")
    @patch("subprocess.run")
    def test_run_pm2_success(self, mock_run, mock_get):
        mock_run.return_value = _make_run_result(stdout="OK\n", returncode=0)
        from tarunai_connect.pm2.utils.pm2_backend import run_pm2
        result = run_pm2("save")
    ...
    @patch("tarunai_connect.pm2.utils.pm2_backend._get_pm2", return_value="/nonexistent/pm2")
...
class TestProcesses:
    ...
    @patch("tarunai_connect.pm2.core.processes.pm2_jlist")
    def test_list_processes_json(self, mock_jlist):
        mock_jlist.return_value = json.loads(FAKE_JLIST)
        from tarunai_connect.pm2.core.processes import list_processes
    ...
    @patch("tarunai_connect.pm2.core.processes.pm2_describe")
    def test_describe_process_found(self, mock_desc):
        mock_desc.return_value = json.loads(FAKE_JLIST)[0]
        from tarunai_connect.pm2.core.processes import describe_process
    ...
    def test_get_metrics_json(self, mock_jlist):
        mock_jlist.return_value = json.loads(FAKE_JLIST)
        from tarunai_connect.pm2.core.processes import get_metrics
...
class TestLifecycle:
    ...
    @patch("tarunai_connect.pm2.core.lifecycle.pm2_action")
    def test_restart_success(self, mock_action):
        mock_action.return_value = {
            "success": True, "returncode": 0,
            "stdout": "restarted", "stderr": "",
        }
        from tarunai_connect.pm2.core.lifecycle import restart_process
    ...
    def test_stop_failure(self, mock_action):
        ...
        from tarunai_connect.pm2.core.lifecycle import stop_process
        result = stop_process("ghost", as_json=False)
        assert result["success"] is False
        assert "Failed" in result["message"]

    @patch("tarunai_connect.pm2.core.lifecycle.backend_start")
    def test_start_with_name(self, mock_start):
        mock_start.return_value = {
            "success": True, "returncode": 0,
            "stdout": "started", "stderr": "",
        }
        from tarunai_connect.pm2.core.lifecycle import start_process
...
class TestLogs:
    ...
    @patch("tarunai_connect.pm2.core.logs.backend_logs")
    def test_view_logs_success(self, mock_logs):
        mock_logs.return_value = {
            "success": True, "returncode": 0,
            "stdout": "line1\nline2\n", "stderr": "",
        }
        from tarunai_connect.pm2.core.logs import view_logs
        result = view_logs("seaclip-dev", lines=20, as_json=False)
        assert result["success"] is True
        assert "line1" in result["content"]

    @patch("tarunai_connect.pm2.core.logs.backend_flush")
    def test_flush_all(self, mock_flush):
        mock_flush.return_value = {
            "success": True, "returncode": 0,
            "stdout": "flushed", "stderr": "",
        }
        from tarunai_connect.pm2.core.logs import flush_logs
...
class TestSystem:
    ...
    @patch("tarunai_connect.pm2.core.system.backend_version")
    def test_version_json(self, mock_ver):
        mock_ver.return_value = "5.3.0"
        from tarunai_connect.pm2.core.system import version
        result = version(as_json=True)
        assert result["version"] == "5.3.0"

    @patch("tarunai_connect.pm2.core.system.backend_save")
    def test_save_success(self, mock_save):
        mock_save.return_value = {
            "success": True, "returncode": 0,
            "stdout": "saved", "stderr": "",
        }
        from tarunai_connect.pm2.core.system import save
...
class TestOutputFormatting:
    ...
    def test_output_json_string(self, capsys):
        from tarunai_connect.pm2.pm2_cli import _output
...
class TestFormatBytes:
    ...
    def test_zero_bytes(self):
        from tarunai_connect.pm2.core.processes import _format_bytes
