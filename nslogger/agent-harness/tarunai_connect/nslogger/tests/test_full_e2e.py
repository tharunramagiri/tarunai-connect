"""End-to-end tests for tarunai-connect-nslogger using real files and subprocess."""
...
from tarunai_connect.nslogger.utils.generate import generate_sample_file
from tarunai_connect.nslogger.core.parser import parse_raw_file
...
from tarunai_connect.nslogger.core.exporter import export_messages
...
def _resolve_cli(name: str) -> list[str]:
    ...
    return [sys.executable, "-m", f"tarunai_connect.nslogger.nslogger_cli"]


def run_cli(*args, expect_ok=True) -> subprocess.CompletedProcess:
    cmd = _resolve_cli("tarunai-connect-nslogger") + list(args)
...
class TestCLISubprocess:
    def test_installed_cli_help(self):
        cmd = _resolve_cli("tarunai-connect-nslogger") + ["--help"]
        result = subprocess.run(cmd, capture_output=True, text=True)
...
class TestClientsCommand:
    ...
    def test_clients_empty_file(self, tmp_path):
        ...
        from tarunai_connect.nslogger.utils.generate import encode_message
...
class TestBlocksCommand:
    ...
    def test_blocks_indent_applied(self, tmp_path):
        from tarunai_connect.nslogger.utils.generate import encode_message
        from tarunai_connect.nslogger.core.message import MSG_TYPE_BLOCK_START, MSG_TYPE_BLOCK_END
