...
import tarunai_connect.nslogger.nslogger_cli as nslogger_cli
from tarunai_connect.nslogger.core.message import (
    LogMessage, MSG_TYPE_LOG, MSG_TYPE_CLIENT_INFO, MSG_TYPE_BLOCK_START, MSG_TYPE_BLOCK_END,
    LEVEL_NAMES,
)
from tarunai_connect.nslogger.core.filter import filter_messages
from tarunai_connect.nslogger.core.stats import compute_stats
from tarunai_connect.nslogger.core.exporter import export_text, export_json, export_csv, export_messages
from tarunai_connect.nslogger.core.blocks import iter_block_tree, extract_clients, merge_files
from tarunai_connect.nslogger.core.listener import (
...
from tarunai_connect.nslogger.nslogger_cli import (
    _format_live_output_message,
    _listen_waiting_message,
    _open_live_output_file,
)
from tarunai_connect.nslogger.utils.generate import encode_message, generate_sample_file
from tarunai_connect.nslogger.core.parser import _parse_message, parse_raw_file
...
class TestNSLoggerListener:
    ...
    def test_macos_bonjour_prefers_native_netservice(self):
        ...
        with patch("tarunai_connect.nslogger.core.listener.sys.platform", "darwin"), \
                patch("tarunai_connect.nslogger.core.listener._NativeBonjourPublisher", DummyNativePublisher), \
                patch("tarunai_connect.nslogger.core.listener._DnsSdBonjourPublisher") as dns_sd_publisher, \
                patch("tarunai_connect.nslogger.core.listener._ZeroconfBonjourPublisher") as zeroconf_publisher:
    ...
    def test_dns_sd_publisher_can_be_forced_on_macos(self):
        ...
        with patch("tarunai_connect.nslogger.core.listener.sys.platform", "darwin"), \
                patch("tarunai_connect.nslogger.core.listener._NativeBonjourPublisher") as native_publisher, \
                patch("tarunai_connect.nslogger.core.listener._DnsSdBonjourPublisher", DummyDnsSdPublisher):
            publisher = listener._start_bonjour("192.168.10.1")
    ...
    def test_zeroconf_publisher_can_be_forced_on_macos(self):
        ...
        with patch("tarunai_connect.nslogger.core.listener.sys.platform", "darwin"), \
                patch("tarunai_connect.nslogger.core.listener._ZeroconfBonjourPublisher", DummyZeroconfPublisher), \
                patch("tarunai_connect.nslogger.core.listener._DnsSdBonjourPublisher") as dns_sd_publisher:
