"""tarunai-connect-nslogger — CLI harness for NSLogger."""
...
@click.version_option(package_name="tarunai-connect-nslogger")
@click.pass_context
def cli(ctx):
    ...
    Use COMMAND -h to see command-specific options, for example:
      tarunai-connect-nslogger listen -h
...
def listen(
    ...
    \b
    TCP mode (default):
      tarunai-connect-nslogger listen --port 50000
...
def _run_repl(ctx, file: Optional[str] = None):
    """Launch the shared tarunai-connect REPL and dispatch commands through Click."""
