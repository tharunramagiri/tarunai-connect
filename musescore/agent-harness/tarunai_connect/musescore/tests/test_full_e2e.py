"""End-to-end tests for tarunai-connect-musescore.
...
Run with: pytest tarunai_connect/musescore/tests/test_full_e2e.py -v -s
"""
...
def _resolve_cli(name: str) -> list[str]:
    ...
    If TARUNAI_CONNECT_FORCE_INSTALLED is set, use the installed command.
    Otherwise, use python -m.
    """
    if os.environ.get("TARUNAI_CONNECT_FORCE_INSTALLED"):
        import shutil
        path = shutil.which(name)
        if path:
            return [path]
        raise RuntimeError(f"{name} not found on PATH")
    return [sys.executable, "-m", "tarunai_connect.musescore"]


def _has_mscore() -> bool:
    """Check if mscore is available."""
    try:
        from tarunai_connect.musescore.utils.musescore_backend import find_musescore
        find_musescore()
        return True
    except RuntimeError:
        return False
...
class TestExportE2E:
    def test_export_pdf(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".pdf", delete=False) as f:
            out = f.name
        try:
            from tarunai_connect.musescore.core.export import export_score, verify_output
    ...
    def test_export_musicxml(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".musicxml", delete=False) as f:
            out = f.name
        try:
            from tarunai_connect.musescore.core.export import export_score
...
class TestTransposeE2E:
    def test_transpose_g_to_c(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".mscz", delete=False) as f:
            out = f.name
        try:
            from tarunai_connect.musescore.core.transpose import transpose_by_key
            ...
            from tarunai_connect.musescore.utils.mscx_xml import read_score_tree, get_key_signature
    ...
    def test_transpose_by_interval(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".mscz", delete=False) as f:
            out = f.name
        try:
            from tarunai_connect.musescore.core.transpose import transpose_by_interval
...
class TestPartsE2E:
    def test_list_parts(self):
        ...
        from tarunai_connect.musescore.core.parts import list_parts
    ...
    def test_extract_part(self):
        ...
        from tarunai_connect.musescore.core.parts import list_parts, extract_part
        parts = list_parts(str(_SAMPLE_MXL))
        if not parts:
...
class TestMediaE2E:
    def test_probe_mxl(self):
        ...
        from tarunai_connect.musescore.core.media import probe_score
    ...
    def test_stats_mxl(self):
        ...
        from tarunai_connect.musescore.core.media import score_stats
...
class TestCLISubprocess:
    def test_help(self):
        """Test --help flag."""
        cmd = _resolve_cli("tarunai-connect-musescore") + ["--help"]
    ...
    def test_json_project_info(self):
        ...
        cmd = _resolve_cli("tarunai-connect-musescore") + [
