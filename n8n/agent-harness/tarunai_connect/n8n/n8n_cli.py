"""tarunai-connect-n8n — CLI for n8n workflow automation.
...
from tarunai_connect.n8n.utils.repl_skin import error, output, print_banner, success, warn
...
@click.version_option(version=VERSION, prog_name="tarunai-connect-n8n")
@click.pass_context
...
def config_test(ctx: click.Context) -> None:
    ...
    if not conn["base_url"]:
        error("No URL configured. Run: tarunai-connect-n8n config set base_url https://...")
        return
    if not conn["api_key"]:
        error("No API key configured. Run: tarunai-connect-n8n config set api_key YOUR_KEY")
        return
    try:
...
def install_completions(shell: str) -> None:
    ...
    import subprocess
    env_var = "_TARUNAI_CONNECT_N8N_COMPLETE"
    shell_map = {"bash": "bash_source", "zsh": "zsh_source", "fish": "fish_source"}
    import os
    result = subprocess.run(
        [sys.executable, "-m", "tarunai_connect.n8n"],
    ...
    if result.stdout:
        ...
        if shell == "bash":
            click.echo('# tarunai-connect-n8n completions bash >> ~/.bashrc', err=True)
        elif shell == "zsh":
            click.echo('# tarunai-connect-n8n completions zsh >> ~/.zshrc', err=True)
        elif shell == "fish":
            click.echo('# tarunai-connect-n8n completions fish > ~/.config/fish/completions/tarunai-connect-n8n.fish', err=True)
    else:
...
def workflow_test(ctx: click.Context, workflow_id: str, test_data: str | None) -> None:
    ...
    if not wf.get("active"):
        error(f"Workflow is not active. Run: tarunai-connect-n8n workflow activate {workflow_id}")
        return
    ...
    from tarunai_connect.n8n.utils.n8n_backend import DEFAULT_TIMEOUT
...
def node_search(ctx: click.Context, query: str, limit: int) -> None:
    ...
    click.echo("\n  Use: tarunai-connect-n8n node info <package-name> for details")
    click.echo()
...
def workflow_patterns(ctx: click.Context) -> None:
    ...
    click.echo("\n  Usage: tarunai-connect-n8n workflow scaffold <pattern> [--deploy]")
    click.echo()
