...
from tarunai_connect.n8n.core import credentials, tags, variables, workflows
...
def _resolve_cli() -> list[str]:
    import shutil
    if shutil.which("tarunai-connect-n8n"):
        return ["tarunai-connect-n8n"]
    return [sys.executable, "-m", "tarunai_connect.n8n"]
...
class TestWorkflowsE2E:
    ...
    def test_workflow_crud(self):
        ...
        wf = workflows.create_workflow(
            {"name": "tarunAI Connect Test WF", "nodes": [], "connections": {}, "settings": {}},
        ...
        try:
            ...
            assert detail["name"] == "tarunAI Connect Test WF"

            updated = workflows.update_workflow(
                wf_id,
                {"name": "tarunAI Connect Test WF Updated", "nodes": [], "connections": {}, "settings": {}},
                base_url=N8N_URL, api_key=N8N_KEY,
            )
            assert updated["name"] == "tarunAI Connect Test WF Updated"
        finally:
