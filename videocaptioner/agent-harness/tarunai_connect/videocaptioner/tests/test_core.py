...
class TestTranscribe:
    @patch("tarunai_connect.videocaptioner.core.transcribe.run_quiet", return_value="/tmp/o.srt")
    def test_basic(self, mock_run):
        from tarunai_connect.videocaptioner.core.transcribe import transcribe
        assert transcribe("video.mp4") == "/tmp/o.srt"
        assert "transcribe" in mock_run.call_args[0][0]
        assert "bijian" in mock_run.call_args[0][0]

    @patch("tarunai_connect.videocaptioner.core.transcribe.run_quiet", return_value="/tmp/o.json")
...
class TestSubtitle:
    @patch("tarunai_connect.videocaptioner.core.subtitle.run_quiet", return_value="/tmp/o.srt")
    def test_translate(self, mock_run):
        from tarunai_connect.videocaptioner.core.subtitle import process_subtitle
...
class TestSynthesize:
    @patch("tarunai_connect.videocaptioner.core.synthesize.run_quiet", return_value="/tmp/o.mp4")
    def test_soft(self, mock_run):
        from tarunai_connect.videocaptioner.core.synthesize import synthesize
    ...
    @patch("tarunai_connect.videocaptioner.core.synthesize.ensure_subtitle_consistency")
    @patch("tarunai_connect.videocaptioner.core.synthesize.run_quiet", return_value="/tmp/o.mp4")
...
class TestPipeline:
    @patch("tarunai_connect.videocaptioner.core.pipeline.run_quiet", return_value="/tmp/o.mp4")
    def test_full(self, mock_run):
        from tarunai_connect.videocaptioner.core.pipeline import process
    ...
    @patch("tarunai_connect.videocaptioner.core.pipeline.run_quiet", return_value="/tmp/o.srt")
...
class TestBackend:
    @patch("tarunai_connect.videocaptioner.utils.vc_backend._find_vc", return_value="/usr/bin/videocaptioner")
    @patch("subprocess.run")
    def test_success(self, mock_sub, _mock_find):
        mock_sub.return_value = MagicMock(returncode=0, stdout="/tmp/o.srt\n", stderr="")
        from tarunai_connect.videocaptioner.utils.vc_backend import run_quiet
    ...
    def test_not_installed(self, _):
        from tarunai_connect.videocaptioner.utils.vc_backend import _find_vc
        with pytest.raises(RuntimeError, match="not found"):
            _find_vc()

    @patch("tarunai_connect.videocaptioner.utils.vc_backend.has_subcommand", return_value=False)
    @patch("tarunai_connect.videocaptioner.utils.vc_backend.get_version", return_value="videocaptioner 1.4.1")
    def test_get_styles_fallback(self, _mock_version, _mock_has_subcommand):
        from tarunai_connect.videocaptioner.utils.vc_backend import get_styles
        styles = get_styles()
        assert "does not expose a 'style' subcommand" in styles

    def test_runtime_guidance(self):
        from tarunai_connect.videocaptioner.utils.vc_backend import get_runtime_guidance
...
class TestReview:
    def test_review_report_passes_for_matching_script(self, tmp_path):
        ...
        from tarunai_connect.videocaptioner.core.review import build_review_report
    ...
    def test_review_report_flags_mismatch(self, tmp_path):
        ...
        from tarunai_connect.videocaptioner.core.review import ensure_subtitle_consistency
    ...
    def test_render_preview_frame(self, mock_run, _mock_which, tmp_path):
        ...
        from tarunai_connect.videocaptioner.core.review import render_preview_frame
