"""Shared helpers for tarunAI Connect preview bundles."""
...
def bundle_root(
    ...
    if root_dir:
        ...
        base = Path(project_path).expanduser().resolve().parent / ".tarunai-connect" / "previews"
    else:
        base = Path.home() / ".tarunai-connect" / "previews"
...
def find_latest_manifest(
    ...
    if root_dir:
        ...
        search_root = Path(project_path).expanduser().resolve().parent / ".tarunai-connect" / "previews" / _slug(software)
    else:
        search_root = Path.home() / ".tarunai-connect" / "previews" / _slug(software)
    if recipe:
