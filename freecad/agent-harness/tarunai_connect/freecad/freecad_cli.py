#!/usr/bin/env python3
"""tarunai-connect-freecad — CLI harness for FreeCAD parametric 3D CAD modeler.
...
from tarunai_connect.freecad.core import (
    document as doc_mod,
    parts as parts_mod,
...
def _spawn_live_poller(session_dir: str) -> dict:
    ...
    cli_path = shutil.which("tarunai-connect-freecad")
    if cli_path:
        ...
        command = [
            sys.executable,
            "-m",
            "tarunai_connect.freecad.freecad_cli",
            "preview",
            "live",
            "monitor",
            "--session-dir",
...
def cli(ctx: click.Context, use_json: bool, project: Optional[str], dry_run: bool) -> None:
    """tarunai-connect-freecad — CLI harness for FreeCAD 3D CAD modeler."""
    global _json_output
...
def preview_capture(recipe: str, force: bool, root_dir: Optional[str]) -> None:
    ...
    result = preview_mod.capture(
        ...
        force=force,
        root_dir=root_dir,
        command=f"tarunai-connect-freecad --project {sess.project_path or ''} preview capture --recipe {recipe}".strip(),
    )
...
def preview_live_start(
    ...
    result = preview_mod.live_start(
        ...
        command=(
            f"tarunai-connect-freecad -p {sess.project_path or ''} "
...
def repl(project_path: Optional[str]) -> None:
    ...
    from tarunai_connect.freecad.utils.repl_skin import ReplSkin
