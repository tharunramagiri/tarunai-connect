"""
Full end-to-end tests for the tarunai-connect-freecad harness.
...
# ---------------------------------------------------------------------------
from tarunai_connect.freecad.core.document import (
    create_document,
    open_document,
    save_document,
    get_document_info,
)
from tarunai_connect.freecad.core.parts import (
    add_part,
    list_parts,
    get_part,
    boolean_op,
...
from tarunai_connect.freecad.core.sketch import (
    create_sketch,
    add_line,
    add_circle,
    add_rectangle,
...
from tarunai_connect.freecad.core.body import (
    additive_box,
    additive_cone,
    additive_cylinder,
    create_body,
...
from tarunai_connect.freecad.core.materials import (
    create_material,
    assign_material,
    list_materials,
)
from tarunai_connect.freecad.core.export import export_project, get_export_info
from tarunai_connect.freecad.core import preview as preview_mod
from tarunai_connect.freecad.core.session import Session
from tarunai_connect.freecad.utils.freecad_macro_gen import generate_macro
...
def _has_freecad() -> bool:
    ...
    try:
        from tarunai_connect.freecad.utils.freecad_backend import find_freecad
        find_freecad()
        return True
...
def _resolve_cli(name: str) -> List[str]:
    ...
    unless ``TARUNAI_CONNECT_FORCE_INSTALLED=1`` is set.
    """
    import shutil
    ...
    module = (
        name.replace("tarunai-connect-", "tarunai_connect.")
        .replace("-", "_")
        + "."
        + name.split("-")[-1]
        + "_cli"
...
class TestFreeCADBackend:
    ...
    def test_get_version(self):
        ...
        from tarunai_connect.freecad.utils.freecad_backend import get_version
...
class TestCLISubprocess:
    ...
    def _cli_cmd(self):
        ...
        self.cli = _resolve_cli("tarunai-connect-freecad")
